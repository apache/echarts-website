...
*   `'cartesian2d'`
    ...
    Lay out based on a two-dimensional [rectangular coordinate system (also known as Cartesian coordinate system)](option.grid.md). When multiple `xAxis` or multiple `yAxis` exist within an ECharts instance, the corresponding axes should be specified using [xAxisIndex](option.series-lines.md#xAxisIndex) and [yAxisIndex](option.series-lines.md#yAxisIndex) or [xAxisId](option.series-lines.md#xAxisId) and [yAxisId](option.series-lines.md#yAxisId).
...
*   `'geo'`
    ...
    Lay out based on a [geographic coordinate system](option.geo.md). When multiple geographic coordinate systems exist within an ECharts instance, the corresponding system should be specified using [geoIndex](option.series-lines.md#geoIndex) or [geoId](option.series-lines.md#geoId).
    

*   `'singleAxis'`
    
    Lay out based on a [singleAxis coordinate system](option.singleAxis.md). When multiple singleAxis coordinate systems exist within an ECharts instance, the corresponding system should be specified using [singleAxisIndex](option.series-lines.md#polarIndex) or [singleAxisId](option.series-lines.md#polarId).
...
See also [series-lines.coordinateSystemUsage](option.series-lines.md#coordinateSystemUsage).
...
Specify how to lay out this `series-lines` based on the specified [coordinateSystem](option.series-lines.md#coordinateSystem).
...
*   `'box'`: **(Not applicable in [series-lines](option.series-lines.md))**
    ...
    *   For example, a [grid component](option.grid.md) can be laid out in a [matrix coordinate system](option.matrix.md) or a [calendar coordinate system](option.calendar.md), where its layout rectangle is calculated by the specified [series-lines.coords](option.series-lines.md#coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-lines.coordinateSystem](option.series-lines.md#coordinateSystem).
...
When [coordinateSystemUsage](option.series-lines.md#coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](option.series-lines.md#coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.
...
If it is set true, [data.coords](option.series-lines.md#data.coords) can have more than two coord to draw a polyline. It is useful when visualizing GPS track data. See example [lines-bus](https://echarts.apache.org/examples/en/editor.html?c=lines-bmap-bus).
...
**Tips:** All the graphs with trail effect should be put on a individual layer, which means that [zlevel](option.series-lines.md#zlevel) need to be different with others. And the animation ([animation](option.series-lines.md#animation): false) of this layer is suggested to be turned off at the meanwhile. Otherwise, other graphic elements in other series and the [label](option.series-lines.md#label) of animation would produce unnecessary ghosts.
...
If symbol movement of special effect has a constant speed, which unit is pixel per second. [period](option.series-lines.md#effect.period) will be ignored if `constantSpeed` is larger than 0.
...
The color of special effect symbol, which defaults to be same with [lineStyle.color](option.series-lines.md#lineStyle.color).
...
After being enabled, [largeThreshold](option.series-lines.md#largeThreshold) can be used to indicate the minimum number for turning on the optimization.
...
Symbol type at the two ends of the line. It can be an array for two ends, or assigned separately. See [data.symbol](option.series-line.md#markLine.data.0.symbol) for more format information.
...
Line color. Color is taken from [option.color Palette](../option.md#color) by default.
...
Label settings. Does not work when [polyline](option.series-lines.md#polyline) is `true`.
...
Configurations of blur state. Available when [emphasis.focus](option.series-lines.md#emphasis.focus) is set.
...
Configurations of select state. Available when [selectedMode](option.series-lines.md#selectedMode) is set.
...
An array includes two ore more than two coordinates. Each coordinate could be `[x, y]` in [rectangular coordinate](option.grid.md) and `[lng, lat]` in [geographic coordinate](option.geo.md).
...
Label of a single line. Available when [polyline](option.series-lines.md#polyline) is not `true`.
...
The first parameter `value` is the value in [data](option.series-lines.md#data), and the second parameter `params` is the rest parameters of data item.
...
1.  Assign coordinate according to container with [x](option.series-lines.md#markPoint.data.x), [y](option.series-lines.md#markPoint.data.y) attribute, in which pixel values and percentage are supported.
...
Mark point name. Can be used as a template variable **data name** `{b}` in [label formatter](option.series-lines.md#markPoint.data.label.formatter).
...
Options: `'container'` or `'coordinate'`. Specify whether the positions of [x](option.series-lines.md#markPoint.data.x) and [y](option.series-lines.md#markPoint.data.y) are relative to the entire chart container (`'container'`, default) or the grid coordinate system (`'coordinate'`).
...
Symbol type at the two ends of the mark line. It can be an array for two ends, or assigned separately. See [data.symbol](option.series-lines.md#markLine.data.0.symbol) for more format information.
...
1.  Assign coordinate according to container with [x](option.series-lines.md#markLine.data.0.x), [y](option.series-lines.md#markLine.data.0.y) attribute, in which pixel values and percentage are supported.
...
Name of the marker. Can be used as a template variable **data name** `{b}` in [label formatter](option.series-lines.md#markLine.data.label.formatter).
...
1.  Specify the coordinate in screen coordinate system using [x](option.series-lines.md#markArea.data.0.x), [y](option.series-lines.md#markArea.data.0.y), where the unit is pixel (e.g., the value is `5`), or percent (e.g., the value is `'35%'`).
...
Name of the marker, which will display as a label. Can be used as a template variable **data name** `{b}` in [label formatter](option.series-lines.md#markArea.data.label.formatter).
