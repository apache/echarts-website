...
See also [boundaryGap](option.radiusAxis.md#boundaryGap). The functionality of the two options overlaps for historicall reasons. Series shapes may overflow a category axis (`axis.type: 'category'`) only if `boundaryGap: false, containShape: false`.
...
It is available only for value, logarithmic, and time axes, i.e., [type](option.radiusAxis.md#type): 'value', 'log', or 'time'.
...
**The difference from [min](option.radiusAxis.md#min):**
...
**The difference from [max](option.radiusAxis.md#max):**
