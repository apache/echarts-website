...
The first x axis in grid defaults to be on the bottom of the grid, and the second x axis is on the other side against the first x axis. Notice: Set `xAxis.axisLine.onZero` to `false` to activate this option.

## offset
- **Type**: `number`
- **Default**: `0`

Offset of x axis relative to default position. Useful when multiple x axis has same [position](option.xAxis.md#position) value. Notice: Set `xAxis.axisLine.onZero` to `false` to activate this option.
...
*   `'category'` Category axis, suitable for discrete category data. Category data can be auto retrieved from [series.data](../option.md#series.data) or [dataset.source](option.dataset.md#source), or can be specified via [xAxis.data](option.xAxis.md#data).
...
Color of axis name uses [axisLine.lineStyle.color](option.xAxis.md#axisLine.lineStyle.color) by default.
...
The `boundaryGap` of category axis can be set to either `true` or `false`. Default value is set to be `true`, in which case [axisTick](option.xAxis.md#axisTick) is served only as a separation line, and labels and data appear only in the center part of two [axis ticks](option.xAxis.md#axisTick), which is called _band_.

For non-category axis, including time, numerical value, and log axes, `boundaryGap` is an array of two values, representing the spanning range between minimum and maximum value. The value can be set in numeric value or relative percentage, which becomes invalid after setting [min](option.xAxis.md#min) and [max](option.xAxis.md#max). **Example:**
...
See also [boundaryGap](option.xAxis.md#boundaryGap). The functionality of the two options overlaps for historicall reasons. Series shapes may overflow a category axis (`axis.type: 'category'`) only if `boundaryGap: false, containShape: false`.
...
It is available only for value, logarithmic, and time axes, i.e., [type](option.xAxis.md#type): 'value', 'log', or 'time'.
...
**The difference from [min](option.xAxis.md#min):**
...
**The difference from [max](option.xAxis.md#max):**
...
It is available only in numerical axis, i.e., [type](option.xAxis.md#type): 'value'.
...
This configuration item is unavailable when the [min](option.xAxis.md#min) and [max](option.xAxis.md#max) are set.
...
It is available only for axis of [type](option.xAxis.md#type) 'value' or 'time'.
...
For example, in time axis ([type](option.xAxis.md#type) is 'time'), it can be set to be `3600 * 24 * 1000` to make sure that the gap between axis labels is less than or equal to one day.
...
As [splitNumber](option.xAxis.md#splitNumber) is a recommendation value, the calculated tick may not be the same as expected. In this case, interval should be used along with [min](option.xAxis.md#min) and [max](option.xAxis.md#max) to compulsively set tickings. But in most cases, we do not suggest using this, our automatic calculation is enough for most situations.

This is unavailable for 'category' and 'time' axes. Logged value should be passed for [type](option.xAxis.md#type): 'log' axis.

## logBase
- **Type**: `number`
- **Default**: `10`

Base of logarithm, which is valid only for numeric axes with [type](option.xAxis.md#type): 'log'.
...
Whether allow overlaping with [jitter](option.xAxis.md#jitter). If `false`, it will try to avoid overlap. But in extreme cases, some scatters may also overlap if there is no way to avoid. The following is the effect of seeting it to be `false`:
...
When setting [jitter](option.xAxis.md#jitter) and [jitterOverlap](option.xAxis.md#jitterOverlap) is `false`, the minimum distance between two scatters.
...
> 
> Axis breaks cannot be used in category axes ([type](option.xAxis.md#type): `'category'`).
...
*   If [axis.type](option.xAxis.md#type) is `'value'` or `'log'`, use `number` type values.
    
*   If [axis.type](option.xAxis.md#type) is `'category'`: not supported yet.
    
*   If [axis.type](option.xAxis.md#type) is `'time'`, the value can be:
...
Note: [xAxis.breaks.start](option.xAxis.md#breaks.start) and [xAxis.breaks.end](option.xAxis.md#breaks.end) are the unique identifiers for each break item. When calling [chart.setOption](../api-parts/api.echartsInstance.md#setOption) to modify [xAxis.breaks.gap](option.xAxis.md#breaks.gap) or [xAxis.breaks.isExpanded](option.xAxis.md#breaks.isExpanded), `start` and `end` must be specified. Update animations will only occur if `start` and `end` are not modified; no animation will occur if they are changed.
...
*   Percentage (string):
    ...
    Specifies the proportion relative to the axis. For example, `'5%'` means that the final size of the axis break area will always be `'5%'` of the axis length. Using a percentage ensures that the pixel size of the axis break area remains stable, and does not change when [xAxis.min](option.xAxis.md#min), [xAxis.max](option.xAxis.md#max), or [dataZoom](../option.md#dataZoom) are modified. For this reason, using a percentage is recommended in most scenarios.
    
*   Absolute value:
    
    Its unit is the same as [start](option.xAxis.md#breaks.start) and [end](option.xAxis.md#breaks.end), referring to a value in the data domain defined by the business data (`series.data`), rather than pixels. It represents mapping (replacing) the `[start, end]` interval with `[start, start + gap]`. Therefore, if set as an absolute value, the pixel size of the axis break area will change when [xAxis.min](option.xAxis.md#min), [xAxis.max](option.xAxis.md#max), or [dataZoom](../option.md#dataZoom) are modified.
    

**Notice:** Within a [xAxis.breaks](option.xAxis.md#breaks) array, `gap` must be specified either entirely in percentages or entirely in absolute values. Mixing the two is not allowed, as it may lead to unexpected results.
...
See also the introduction to the axis break in [xAxis.breaks](../option.md#xAxisbreaks).
...
Interval of axisTick, which is available in category axis. is set to be the same as [axisLabel.interval](option.xAxis.md#axisLabel.interval) by default.
...
Color of axis label is set to be [axisLine.lineStyle.color](option.xAxis.md#axisLine.lineStyle.color) by default.
...
Style configuration of minor ticks lines [axisLine.lineStyle.color](option.xAxis.md#axisLine.lineStyle.color)。
...
**When [axis break](xAxis.breaks) is used**
...
**For a time axis ([`xAxis.type: 'time'`](option.xAxis.md#type))**
...
Alignment of the label of the max tick. If set to be `null`, it's the same with other labels. See [align](option.xAxis.md#axisLabel.align).
...
Color of axis label is set to be [axisLine.lineStyle.color](option.xAxis.md#axisLine.lineStyle.color) by default. Callback function is supported, in the following format:
...
Interval of Axis splitLine, which is available in category axis. is set to be the same as [axisLabel.interval](option.xAxis.md#axisLabel.interval) by default.
...
Minor split lines of axis in the [grid](option.grid.md) area。It will align to the [minorTick](option.xAxis.md#minorTick)
...
Interval of Axis splitArea, which is available in category axis. is set to be the same as [axisLabel.interval](option.xAxis.md#axisLabel.interval) by default.
...
The tooltip configuration for xAxis, must set [triggerEvent](option.xAxis.md#triggerEvent) as `true` and enable global [option.tooltip](option.tooltip.md) component.
