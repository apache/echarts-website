...
Specifies another coordinate system component on which this `series-scatter` is laid out.

Options:

*   `'cartesian2d'`
    
    Lay out based on a two-dimensional [rectangular coordinate system (also known as Cartesian coordinate system)](option.grid.md). When multiple `xAxis` or multiple `yAxis` exist within an ECharts instance, the corresponding axes should be specified using [xAxisIndex](option.series-scatter.md#xAxisIndex) and [yAxisIndex](option.series-scatter.md#yAxisIndex) or [xAxisId](option.series-scatter.md#xAxisId) and [yAxisId](option.series-scatter.md#yAxisId).
...
*   `'polar'`
    ...
    Lay out based on a [polar coordinate system](option.polar.md). When multiple polar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [polarIndex](option.series-scatter.md#polarIndex) or [polarId](option.series-scatter.md#polarId).
    

*   `'geo'`
    
    Lay out based on a [geographic coordinate system](option.geo.md). When multiple geographic coordinate systems exist within an ECharts instance, the corresponding system should be specified using [geoIndex](option.series-scatter.md#geoIndex) or [geoId](option.series-scatter.md#geoId).
    

*   `'singleAxis'`
    
    Lay out based on a [singleAxis coordinate system](option.singleAxis.md). When multiple singleAxis coordinate systems exist within an ECharts instance, the corresponding system should be specified using [singleAxisIndex](option.series-scatter.md#polarIndex) or [singleAxisId](option.series-scatter.md#polarId).
    

*   `'calendar'`
    
    Lay out based on a [calendar coordinate system](option.calendar.md). When multiple calendar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [calendarIndex](option.series-scatter.md#calendarIndex) or [calendarId](option.series-scatter.md#calendarId).
    

*   `'matrix'`
    
    Lay out based on a [matrix coordinate system](option.matrix.md). When multiple matrix coordinate systems exist within an ECharts instance, the corresponding system should be specified using [matrixIndex](option.series-scatter.md#matrixIndex) or [matrixId](option.series-scatter.md#matrixId).
...
See also [series-scatter.coordinateSystemUsage](option.series-scatter.md#coordinateSystemUsage).
...
Specify how to lay out this `series-scatter` based on the specified [coordinateSystem](option.series-scatter.md#coordinateSystem).
...
*   `'box'`: **(Not applicable in [series-scatter](option.series-scatter.md))**
    ...
    *   For example, a [grid component](option.grid.md) can be laid out in a [matrix coordinate system](option.matrix.md) or a [calendar coordinate system](option.calendar.md), where its layout rectangle is calculated by the specified [series-scatter.coords](option.series-scatter.md#coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-scatter.coordinateSystem](option.series-scatter.md#coordinateSystem).
...
When [coordinateSystemUsage](option.series-scatter.md#coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](option.series-scatter.md#coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.
...
The first parameter `value` is the value in [data](option.series-scatter.md#data), and the second parameter `params` is the rest parameters of data item.
...
Configurations of blur state. Available when [emphasis.focus](option.series-scatter.md#emphasis.focus) is set.
...
Configurations of select state. Available when [selectedMode](option.series-scatter.md#selectedMode) is set.
...
1.  Assign coordinate according to container with [x](option.series-scatter.md#markPoint.data.x), [y](option.series-scatter.md#markPoint.data.y) attribute, in which pixel values and percentage are supported.

2.  Assign coordinate position with [coord](option.series-scatter.md#markPoint.data.coord) attribute, in which `'min'`, `'max'`, `'average'` are supported for each dimension.
    
3.  Use [type](option.series-scatter.md#markPoint.data.type) attribute to mark the maximum and minimum values in the series, in which [valueIndex](option.series-scatter.md#markPoint.data.valueIndex) or [valueDim](option.series-scatter.md#markPoint.data.valueDim) can be used to assign the dimension.
...
Mark point name. Can be used as a template variable **data name** `{b}` in [label formatter](option.series-scatter.md#markPoint.data.label.formatter).
...
Available when using [type](option.series-scatter.md#markPoint.data.type) it is used to assign maximum value and minimum value in dimensions, it could be `0` (xAxis, radiusAxis), `1` (yAxis, angleAxis), and use the first value axis dimension by default.

#### markPoint.data.valueDim
- **Type**: `string`

Works only when [type](option.series-scatter.md#markPoint.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be the direct name of a dimension, like `x`, or `angle` for line charts, or `open`, or `close` for candlestick charts.
...
Options: `'container'` or `'coordinate'`. Specify whether the positions of [x](option.series-scatter.md#markPoint.data.x) and [y](option.series-scatter.md#markPoint.data.y) are relative to the entire chart container (`'container'`, default) or the grid coordinate system (`'coordinate'`).
...
Symbol type at the two ends of the mark line. It can be an array for two ends, or assigned separately. See [data.symbol](option.series-scatter.md#markLine.data.0.symbol) for more format information.
...
1.  Assign coordinate according to container with [x](option.series-scatter.md#markLine.data.0.x), [y](option.series-scatter.md#markLine.data.0.y) attribute, in which pixel values and percentage are supported.

2.  Assign coordinate position with [coord](option.series-scatter.md#markLine.data.0.coord) attribute, in which `'min'`, `'max'`, `'average'` are supported for each dimension.
    
3.  Use [type](option.series-scatter.md#markLine.data.0.type) attribute to mark the maximum and minimum values in the series, in which [valueIndex](option.series-scatter.md#markLine.data.0.valueIndex) or [valueDim](option.series-scatter.md#markLine.data.0.valueDim) can be used to assign the dimension.
...
Works only when [type](option.series-scatter.md#markLine.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be `0` (for xAxis, or radiusAxis), or `1` (for yAxis, or angleAxis). Dimension of the first numeric axis is used by default.

##### markLine.data.0.valueDim
- **Type**: `string`

Works only when [type](option.series-scatter.md#markLine.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be the direct name of a dimension, like `x`, or `angle` for line charts, or `open`, or `close` for candlestick charts.
...
Name of the marker. Can be used as a template variable **data name** `{b}` in [label formatter](option.series-scatter.md#markLine.data.label.formatter).
...
1.  Specify the coordinate in screen coordinate system using [x](option.series-scatter.md#markArea.data.0.x), [y](option.series-scatter.md#markArea.data.0.y), where the unit is pixel (e.g., the value is `5`), or percent (e.g., the value is `'35%'`).

2.  Specify the coordinate in data coordinate system (i.e., cartesian) using [coord](option.series-scatter.md#markArea.data.0.coord), which can be also set as `'min'`, `'max'`, `'average'` (e.g, `coord: [23, 'min']`, or `coord: ['average', 'max']`).
    
3.  Locate the point on the min value or max value of `series.data` using [type](option.series-scatter.md#markArea.data.0.type), where [valueIndex](option.series-scatter.md#markArea.data.0.valueIndex) or [valueDim](option.series-scatter.md#markPoint.data.0.valueDim) can be used to specify the dimension on which the min, max or average are calculated.
...
Specify the dimension on which min, max, average are calculated, available when [type](option.series-scatter.md#markArea.data.type) used. The value can be `0` (means xAxis, radiusAxis) or `1` (means yAxis, angleAxis), using the dimension of the first axis by default.

##### markArea.data.0.valueDim
- **Type**: `string`

Specify the dimension on which min, max, average are calculated, available when [type](option.series-scatter.md#markArea.data.type) used. The value can be the name of the dimension (for example, the value can be `x`, `angle` in line chart, and `open`, `close` in candlestick).
...
Name of the marker, which will display as a label. Can be used as a template variable **data name** `{b}` in [label formatter](option.series-scatter.md#markArea.data.label.formatter).
