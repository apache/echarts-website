...
*   `'calendar'`
    ...
    Lay out based on a [calendar coordinate system](option.calendar.md). When multiple calendar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [calendarIndex](option.parallel.md#calendarIndex) or [calendarId](option.parallel.md#calendarId).
    

*   `'matrix'`
    
    Lay out based on a [matrix coordinate system](option.matrix.md). When multiple matrix coordinate systems exist within an ECharts instance, the corresponding system should be specified using [matrixIndex](option.parallel.md#matrixIndex) or [matrixId](option.parallel.md#matrixId).
...
See also [parallel.coordinateSystemUsage](option.parallel.md#coordinateSystemUsage).
...
Specify how to lay out this `parallel` based on the specified [coordinateSystem](option.parallel.md#coordinateSystem).

In most cases, there is no need to specify `coordinateSystemUsage`, unless the default behavior is unexpected.

Options:

*   `'data'`: **(Not applicable in [parallel](option.parallel.md))**
...
*   `'box'`:
    ...
    *   For example, a [grid component](option.grid.md) can be laid out in a [matrix coordinate system](option.matrix.md) or a [calendar coordinate system](option.calendar.md), where its layout rectangle is calculated by the specified [parallel.coords](option.parallel.md#coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [parallel.coordinateSystem](option.parallel.md#coordinateSystem).
...
When [coordinateSystemUsage](option.parallel.md#coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](option.parallel.md#coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.
...
In this case, you may use [parallel.axisExpandable](option.parallel.md#axisExpandable) to improve the display. See this example:
...
When configuring multiple [parallelAxis](option.parallelAxis.md), there might be some common attributes in each axis configuration. To avoid writing them repeatedly, they can be put under [parallel.parallelAxisDefault](option.parallel.md#parallelAxisDefault). Before initializing axis, configurations in [parallel.parallelAxisDefault](option.parallel.md#parallelAxisDefault) will be merged into [parallelAxis](option.parallelAxis.md) to generate the final axis configuration.
