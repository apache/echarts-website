...
*   `'category'` Category axis, suitable for discrete category data. Category data can be auto retrieved from [series.data](../option.md#series.data) or [dataset.source](option.dataset.md#source), or can be specified via [angleAxis.data](option.angleAxis.md#data).
...
The `boundaryGap` of category axis can be set to either `true` or `false`. Default value is set to be `true`, in which case [axisTick](option.angleAxis.md#axisTick) is served only as a separation line, and labels and data appear only in the center part of two [axis ticks](option.angleAxis.md#axisTick), which is called _band_.

For non-category axis, including time, numerical value, and log axes, `boundaryGap` is an array of two values, representing the spanning range between minimum and maximum value. The value can be set in numeric value or relative percentage, which becomes invalid after setting [min](option.angleAxis.md#min) and [max](option.angleAxis.md#max). **Example:**
...
See also [boundaryGap](option.angleAxis.md#boundaryGap). The functionality of the two options overlaps for historicall reasons. Series shapes may overflow a category axis (`axis.type: 'category'`) only if `boundaryGap: false, containShape: false`.
...
It is available only for value, logarithmic, and time axes, i.e., [type](option.angleAxis.md#type): 'value', 'log', or 'time'.
...
**The difference from [min](option.angleAxis.md#min):**
...
**The difference from [max](option.angleAxis.md#max):**
...
It is available only in numerical axis, i.e., [type](option.angleAxis.md#type): 'value'.
...
This configuration item is unavailable when the [min](option.angleAxis.md#min) and [max](option.angleAxis.md#max) are set.
...
It is available only for axis of [type](option.angleAxis.md#type) 'value' or 'time'.
...
For example, in time axis ([type](option.angleAxis.md#type) is 'time'), it can be set to be `3600 * 24 * 1000` to make sure that the gap between axis labels is less than or equal to one day.
...
As [splitNumber](option.angleAxis.md#splitNumber) is a recommendation value, the calculated tick may not be the same as expected. In this case, interval should be used along with [min](option.angleAxis.md#min) and [max](option.angleAxis.md#max) to compulsively set tickings. But in most cases, we do not suggest using this, our automatic calculation is enough for most situations.

This is unavailable for 'category' and 'time' axes. Logged value should be passed for [type](option.angleAxis.md#type): 'log' axis.

## logBase
- **Type**: `number`
- **Default**: `10`

Base of logarithm, which is valid only for numeric axes with [type](option.angleAxis.md#type): 'log'.
...
Interval of axisTick, which is available in category axis. is set to be the same as [axisLabel.interval](option.angleAxis.md#axisLabel.interval) by default.
...
Color of axis label is set to be [axisLine.lineStyle.color](option.angleAxis.md#axisLine.lineStyle.color) by default.
...
Style configuration of minor ticks lines [axisLine.lineStyle.color](option.angleAxis.md#axisLine.lineStyle.color)。
...
**When [axis break](angleAxis.breaks) is used**
...
**For a time axis ([`angleAxis.type: 'time'`](option.angleAxis.md#type))**
...
Color of axis label is set to be [axisLine.lineStyle.color](option.angleAxis.md#axisLine.lineStyle.color) by default. Callback function is supported, in the following format:
...
Interval of Axis splitLine, which is available in category axis. is set to be the same as [axisLabel.interval](option.angleAxis.md#axisLabel.interval) by default.
...
Minor split lines of axis in the [grid](option.grid.md) area。It will align to the [minorTick](option.angleAxis.md#minorTick)
...
Interval of Axis splitArea, which is available in category axis. is set to be the same as [axisLabel.interval](option.angleAxis.md#axisLabel.interval) by default.
...
The tooltip configuration for angleAxis, must set [triggerEvent](option.angleAxis.md#triggerEvent) as `true` and enable global [option.tooltip](option.tooltip.md) component.
