...
*   `'category'` Category axis, suitable for discrete category data. Category data can be auto retrieved from [series.data](../option.md#series.data) or [dataset.source](option.dataset.md#source), or can be specified via [singleAxis.data](option.singleAxis.md#data).
...
Color of axis name uses [axisLine.lineStyle.color](option.singleAxis.md#axisLine.lineStyle.color) by default.
...
The `boundaryGap` of category axis can be set to either `true` or `false`. Default value is set to be `true`, in which case [axisTick](option.singleAxis.md#axisTick) is served only as a separation line, and labels and data appear only in the center part of two [axis ticks](option.singleAxis.md#axisTick), which is called _band_.

For non-category axis, including time, numerical value, and log axes, `boundaryGap` is an array of two values, representing the spanning range between minimum and maximum value. The value can be set in numeric value or relative percentage, which becomes invalid after setting [min](option.singleAxis.md#min) and [max](option.singleAxis.md#max). **Example:**
...
See also [boundaryGap](option.singleAxis.md#boundaryGap). The functionality of the two options overlaps for historicall reasons. Series shapes may overflow a category axis (`axis.type: 'category'`) only if `boundaryGap: false, containShape: false`.
...
It is available only for value, logarithmic, and time axes, i.e., [type](option.singleAxis.md#type): 'value', 'log', or 'time'.
...
**The difference from [min](option.singleAxis.md#min):**
...
**The difference from [max](option.singleAxis.md#max):**
...
It is available only in numerical axis, i.e., [type](option.singleAxis.md#type): 'value'.
...
This configuration item is unavailable when the [min](option.singleAxis.md#min) and [max](option.singleAxis.md#max) are set.
...
It is available only for axis of [type](option.singleAxis.md#type) 'value' or 'time'.
...
For example, in time axis ([type](option.singleAxis.md#type) is 'time'), it can be set to be `3600 * 24 * 1000` to make sure that the gap between axis labels is less than or equal to one day.
...
As [splitNumber](option.singleAxis.md#splitNumber) is a recommendation value, the calculated tick may not be the same as expected. In this case, interval should be used along with [min](option.singleAxis.md#min) and [max](option.singleAxis.md#max) to compulsively set tickings. But in most cases, we do not suggest using this, our automatic calculation is enough for most situations.

This is unavailable for 'category' and 'time' axes. Logged value should be passed for [type](option.singleAxis.md#type): 'log' axis.

## logBase
- **Type**: `number`
- **Default**: `10`

Base of logarithm, which is valid only for numeric axes with [type](option.singleAxis.md#type): 'log'.
...
Interval of axisTick, which is available in category axis. is set to be the same as [axisLabel.interval](option.singleAxis.md#axisLabel.interval) by default.
...
Color of axis label is set to be [axisLine.lineStyle.color](option.singleAxis.md#axisLine.lineStyle.color) by default.
...
Style configuration of minor ticks lines [axisLine.lineStyle.color](option.singleAxis.md#axisLine.lineStyle.color)。
...
**When [axis break](singleAxis.breaks) is used**
...
**For a time axis ([`singleAxis.type: 'time'`](option.singleAxis.md#type))**
...
Color of axis label is set to be [axisLine.lineStyle.color](option.singleAxis.md#axisLine.lineStyle.color) by default. Callback function is supported, in the following format:
...
Interval of Axis splitLine, which is available in category axis. is set to be the same as [axisLabel.interval](option.singleAxis.md#axisLabel.interval) by default.
...
Minor split lines of axis in the [grid](option.grid.md) area。It will align to the [minorTick](option.singleAxis.md#minorTick)
...
Interval of Axis splitArea, which is available in category axis. is set to be the same as [axisLabel.interval](option.singleAxis.md#axisLabel.interval) by default.
