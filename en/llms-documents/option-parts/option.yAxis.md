...
The first y axis in grid defaults to be the left (`'left'`) of the grid, and the second y axis is on the other side against the first y axis. Notice: Set `yAxis.axisLine.onZero` to `false` to activate this option.

## offset
- **Type**: `number`
- **Default**: `0`

Offset of y axis relative to default position. Useful when multiple y axis has same [position](option.yAxis.md#position) value. Notice: Set `yAxis.axisLine.onZero` to `false` to activate this option.
...
*   `'category'` Category axis, suitable for discrete category data. Category data can be auto retrieved from [series.data](../option.md#series.data) or [dataset.source](option.dataset.md#source), or can be specified via [yAxis.data](option.yAxis.md#data).
...
Color of axis name uses [axisLine.lineStyle.color](option.yAxis.md#axisLine.lineStyle.color) by default.
...
The `boundaryGap` of category axis can be set to either `true` or `false`. Default value is set to be `true`, in which case [axisTick](option.yAxis.md#axisTick) is served only as a separation line, and labels and data appear only in the center part of two [axis ticks](option.yAxis.md#axisTick), which is called _band_.

For non-category axis, including time, numerical value, and log axes, `boundaryGap` is an array of two values, representing the spanning range between minimum and maximum value. The value can be set in numeric value or relative percentage, which becomes invalid after setting [min](option.yAxis.md#min) and [max](option.yAxis.md#max). **Example:**
...
See also [boundaryGap](option.yAxis.md#boundaryGap). The functionality of the two options overlaps for historicall reasons. Series shapes may overflow a category axis (`axis.type: 'category'`) only if `boundaryGap: false, containShape: false`.
...
It is available only for value, logarithmic, and time axes, i.e., [type](option.yAxis.md#type): 'value', 'log', or 'time'.
...
**The difference from [min](option.yAxis.md#min):**
...
**The difference from [max](option.yAxis.md#max):**
...
It is available only in numerical axis, i.e., [type](option.yAxis.md#type): 'value'.
...
This configuration item is unavailable when the [min](option.yAxis.md#min) and [max](option.yAxis.md#max) are set.
...
It is available only for axis of [type](option.yAxis.md#type) 'value' or 'time'.
...
For example, in time axis ([type](option.yAxis.md#type) is 'time'), it can be set to be `3600 * 24 * 1000` to make sure that the gap between axis labels is less than or equal to one day.
...
As [splitNumber](option.yAxis.md#splitNumber) is a recommendation value, the calculated tick may not be the same as expected. In this case, interval should be used along with [min](option.yAxis.md#min) and [max](option.yAxis.md#max) to compulsively set tickings. But in most cases, we do not suggest using this, our automatic calculation is enough for most situations.

This is unavailable for 'category' and 'time' axes. Logged value should be passed for [type](option.yAxis.md#type): 'log' axis.

## logBase
- **Type**: `number`
- **Default**: `10`

Base of logarithm, which is valid only for numeric axes with [type](option.yAxis.md#type): 'log'.
...
Whether allow overlaping with [jitter](option.yAxis.md#jitter). If `false`, it will try to avoid overlap. But in extreme cases, some scatters may also overlap if there is no way to avoid. The following is the effect of seeting it to be `false`:
...
When setting [jitter](option.yAxis.md#jitter) and [jitterOverlap](option.yAxis.md#jitterOverlap) is `false`, the minimum distance between two scatters.
...
> 
> Axis breaks cannot be used in category axes ([type](option.yAxis.md#type): `'category'`).
...
*   If [axis.type](option.yAxis.md#type) is `'value'` or `'log'`, use `number` type values.
    
*   If [axis.type](option.yAxis.md#type) is `'category'`: not supported yet.
    
*   If [axis.type](option.yAxis.md#type) is `'time'`, the value can be:
...
Note: [yAxis.breaks.start](option.yAxis.md#breaks.start) and [yAxis.breaks.end](option.yAxis.md#breaks.end) are the unique identifiers for each break item. When calling [chart.setOption](../api-parts/api.echartsInstance.md#setOption) to modify [yAxis.breaks.gap](option.yAxis.md#breaks.gap) or [yAxis.breaks.isExpanded](option.yAxis.md#breaks.isExpanded), `start` and `end` must be specified. Update animations will only occur if `start` and `end` are not modified; no animation will occur if they are changed.
...
*   Percentage (string):
    ...
    Specifies the proportion relative to the axis. For example, `'5%'` means that the final size of the axis break area will always be `'5%'` of the axis length. Using a percentage ensures that the pixel size of the axis break area remains stable, and does not change when [yAxis.min](option.yAxis.md#min), [yAxis.max](option.yAxis.md#max), or [dataZoom](../option.md#dataZoom) are modified. For this reason, using a percentage is recommended in most scenarios.
    
*   Absolute value:
    
    Its unit is the same as [start](option.yAxis.md#breaks.start) and [end](option.yAxis.md#breaks.end), referring to a value in the data domain defined by the business data (`series.data`), rather than pixels. It represents mapping (replacing) the `[start, end]` interval with `[start, start + gap]`. Therefore, if set as an absolute value, the pixel size of the axis break area will change when [yAxis.min](option.yAxis.md#min), [yAxis.max](option.yAxis.md#max), or [dataZoom](../option.md#dataZoom) are modified.
    

**Notice:** Within a [yAxis.breaks](option.yAxis.md#breaks) array, `gap` must be specified either entirely in percentages or entirely in absolute values. Mixing the two is not allowed, as it may lead to unexpected results.
...
See also the introduction to the axis break in [yAxis.breaks](../option.md#yAxisbreaks).
...
Specifies whether X or Y axis lies on the origin position (i.e., `0` point) of its orthogonal axis. It works only if the orthogonal axis is ['value' | 'log'](option.xAxis.md#type) axis and contains `0`.

Since `v6.1.0` The value 'auto' is introduced
...
Interval of axisTick, which is available in category axis. is set to be the same as [axisLabel.interval](option.yAxis.md#axisLabel.interval) by default.
...
Color of axis label is set to be [axisLine.lineStyle.color](option.yAxis.md#axisLine.lineStyle.color) by default.
...
Style configuration of minor ticks lines [axisLine.lineStyle.color](option.yAxis.md#axisLine.lineStyle.color)。
...
**When [axis break](yAxis.breaks) is used**
...
**For a time axis ([`yAxis.type: 'time'`](option.yAxis.md#type))**
...
Vertical alignment of the label of the min tick. If set to be `null`, it's the same with other labels. See [verticalAlign](option.yAxis.md#axisLabel.verticalAlign).
...
Vertical alignment of the label of the max tick. If set to be `null`, it's the same with other labels. See [verticalAlign](option.yAxis.md#axisLabel.verticalAlign).
...
Color of axis label is set to be [axisLine.lineStyle.color](option.yAxis.md#axisLine.lineStyle.color) by default. Callback function is supported, in the following format:
...
Interval of Axis splitLine, which is available in category axis. is set to be the same as [axisLabel.interval](option.yAxis.md#axisLabel.interval) by default.
...
Minor split lines of axis in the [grid](option.grid.md) area。It will align to the [minorTick](option.yAxis.md#minorTick)
...
Interval of Axis splitArea, which is available in category axis. is set to be the same as [axisLabel.interval](option.yAxis.md#axisLabel.interval) by default.
...
The tooltip configuration for yAxis, must set [triggerEvent](option.yAxis.md#triggerEvent) as `true` and enable global [option.tooltip](option.tooltip.md) component.
