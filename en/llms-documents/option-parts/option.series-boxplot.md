...
Specifies another coordinate system component on which this `series-boxplot` is laid out.

Options:

*   `'cartesian2d'`
    
    Lay out based on a two-dimensional [rectangular coordinate system (also known as Cartesian coordinate system)](option.grid.md). When multiple `xAxis` or multiple `yAxis` exist within an ECharts instance, the corresponding axes should be specified using [xAxisIndex](option.series-boxplot.md#xAxisIndex) and [yAxisIndex](option.series-boxplot.md#yAxisIndex) or [xAxisId](option.series-boxplot.md#xAxisId) and [yAxisId](option.series-boxplot.md#yAxisId).
...
See also [series-boxplot.coordinateSystemUsage](option.series-boxplot.md#coordinateSystemUsage).
...
Specify how to lay out this `series-boxplot` based on the specified [coordinateSystem](option.series-boxplot.md#coordinateSystem).
...
*   `'box'`: **(Not applicable in [series-boxplot](option.series-boxplot.md))**
    ...
    *   For example, a [grid component](option.grid.md) can be laid out in a [matrix coordinate system](option.matrix.md) or a [calendar coordinate system](option.calendar.md), where its layout rectangle is calculated by the specified [series-boxplot.coords](option.series-boxplot.md#coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [series-boxplot.coordinateSystem](option.series-boxplot.md#coordinateSystem).
...
When [coordinateSystemUsage](option.series-boxplot.md#coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](option.series-boxplot.md#coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.
...
boxplot color. Color is taken from [option.color Palette](../option.md#color) by default.
...
Configurations of blur state. Available when [emphasis.focus](option.series-boxplot.md#emphasis.focus) is set.
...
Configurations of select state. Available when [selectedMode](option.series-boxplot.md#selectedMode) is set.
...
The first parameter `value` is the value in [data](option.series-boxplot.md#data), and the second parameter `params` is the rest parameters of data item.
...
1.  Assign coordinate according to container with [x](option.series-boxplot.md#markPoint.data.x), [y](option.series-boxplot.md#markPoint.data.y) attribute, in which pixel values and percentage are supported.

2.  Assign coordinate position with [coord](option.series-boxplot.md#markPoint.data.coord) attribute, in which `'min'`, `'max'`, `'average'` are supported for each dimension.
    
3.  Use [type](option.series-boxplot.md#markPoint.data.type) attribute to mark the maximum and minimum values in the series, in which [valueIndex](option.series-boxplot.md#markPoint.data.valueIndex) or [valueDim](option.series-boxplot.md#markPoint.data.valueDim) can be used to assign the dimension.
...
Mark point name. Can be used as a template variable **data name** `{b}` in [label formatter](option.series-boxplot.md#markPoint.data.label.formatter).
...
Available when using [type](option.series-boxplot.md#markPoint.data.type) it is used to assign maximum value and minimum value in dimensions, it could be `0` (xAxis, radiusAxis), `1` (yAxis, angleAxis), and use the first value axis dimension by default.

#### markPoint.data.valueDim
- **Type**: `string`

Works only when [type](option.series-boxplot.md#markPoint.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be the direct name of a dimension, like `x`, or `angle` for line charts, or `open`, or `close` for candlestick charts.
...
Options: `'container'` or `'coordinate'`. Specify whether the positions of [x](option.series-boxplot.md#markPoint.data.x) and [y](option.series-boxplot.md#markPoint.data.y) are relative to the entire chart container (`'container'`, default) or the grid coordinate system (`'coordinate'`).
...
Symbol type at the two ends of the mark line. It can be an array for two ends, or assigned separately. See [data.symbol](option.series-boxplot.md#markLine.data.0.symbol) for more format information.
...
1.  Assign coordinate according to container with [x](option.series-boxplot.md#markLine.data.0.x), [y](option.series-boxplot.md#markLine.data.0.y) attribute, in which pixel values and percentage are supported.

2.  Assign coordinate position with [coord](option.series-boxplot.md#markLine.data.0.coord) attribute, in which `'min'`, `'max'`, `'average'` are supported for each dimension.
    
3.  Use [type](option.series-boxplot.md#markLine.data.0.type) attribute to mark the maximum and minimum values in the series, in which [valueIndex](option.series-boxplot.md#markLine.data.0.valueIndex) or [valueDim](option.series-boxplot.md#markLine.data.0.valueDim) can be used to assign the dimension.
...
Works only when [type](option.series-boxplot.md#markLine.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be `0` (for xAxis, or radiusAxis), or `1` (for yAxis, or angleAxis). Dimension of the first numeric axis is used by default.

##### markLine.data.0.valueDim
- **Type**: `string`

Works only when [type](option.series-boxplot.md#markLine.data.type) is assigned. It is used to state the dimension used to calculate maximum value or minimum value. It may be the direct name of a dimension, like `x`, or `angle` for line charts, or `open`, or `close` for candlestick charts.
...
Name of the marker. Can be used as a template variable **data name** `{b}` in [label formatter](option.series-boxplot.md#markLine.data.label.formatter).
...
1.  Specify the coordinate in screen coordinate system using [x](option.series-boxplot.md#markArea.data.0.x), [y](option.series-boxplot.md#markArea.data.0.y), where the unit is pixel (e.g., the value is `5`), or percent (e.g., the value is `'35%'`).

2.  Specify the coordinate in data coordinate system (i.e., cartesian) using [coord](option.series-boxplot.md#markArea.data.0.coord), which can be also set as `'min'`, `'max'`, `'average'` (e.g, `coord: [23, 'min']`, or `coord: ['average', 'max']`).
    
3.  Locate the point on the min value or max value of `series.data` using [type](option.series-boxplot.md#markArea.data.0.type), where [valueIndex](option.series-boxplot.md#markArea.data.0.valueIndex) or [valueDim](option.series-boxplot.md#markPoint.data.0.valueDim) can be used to specify the dimension on which the min, max or average are calculated.
...
Specify the dimension on which min, max, average are calculated, available when [type](option.series-boxplot.md#markArea.data.type) used. The value can be `0` (means xAxis, radiusAxis) or `1` (means yAxis, angleAxis), using the dimension of the first axis by default.

##### markArea.data.0.valueDim
- **Type**: `string`

Specify the dimension on which min, max, average are calculated, available when [type](option.series-boxplot.md#markArea.data.type) used. The value can be the name of the dimension (for example, the value can be `x`, `angle` in line chart, and `open`, `close` in candlestick).
...
Name of the marker, which will display as a label. Can be used as a template variable **data name** `{b}` in [label formatter](option.series-boxplot.md#markArea.data.label.formatter).
