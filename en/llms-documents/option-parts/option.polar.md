...
*   `'calendar'`
    ...
    Lay out based on a [calendar coordinate system](option.calendar.md). When multiple calendar coordinate systems exist within an ECharts instance, the corresponding system should be specified using [calendarIndex](option.polar.md#calendarIndex) or [calendarId](option.polar.md#calendarId).
    

*   `'matrix'`
    
    Lay out based on a [matrix coordinate system](option.matrix.md). When multiple matrix coordinate systems exist within an ECharts instance, the corresponding system should be specified using [matrixIndex](option.polar.md#matrixIndex) or [matrixId](option.polar.md#matrixId).
...
See also [polar.coordinateSystemUsage](option.polar.md#coordinateSystemUsage).
...
Specify how to lay out this `polar` based on the specified [coordinateSystem](option.polar.md#coordinateSystem).

In most cases, there is no need to specify `coordinateSystemUsage`, unless the default behavior is unexpected.

Options:

*   `'data'`: **(Not applicable in [polar](option.polar.md))**
...
*   `'box'`:
    ...
    *   For example, a [grid component](option.grid.md) can be laid out in a [matrix coordinate system](option.matrix.md) or a [calendar coordinate system](option.calendar.md), where its layout rectangle is calculated by the specified [polar.coords](option.polar.md#coords) in that system. See example [sparkline in matrix](https://echarts.apache.org/examples/en/editor.html?c=matrix-sparkline&edit=1&reset=1).
...
See also [polar.coordinateSystem](option.polar.md#coordinateSystem).
...
When [coordinateSystemUsage](option.polar.md#coordinateSystemUsage) is `'box'`, `coord` is used as the input to the coordinate system and calculate the layout rectangle or anchor point.
...
> Note: when [coordinateSystemUsage](option.polar.md#coordinateSystemUsage) is `'data'`, the input of coordinate system is `series.data[i]` rather than this `coord`.
