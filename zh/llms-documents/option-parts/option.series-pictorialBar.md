...
指定另一个坐标系组件，本 `series-pictorialBar` 布局在那个坐标系中。

可选值：

*   `'cartesian2d'`
    
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](option.grid.md) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](option.series-pictorialBar.md#xAxisIndex) 和 [yAxisIndex](option.series-pictorialBar.md#yAxisIndex) 或者 [xAxisId](option.series-pictorialBar.md#xAxisId) 和 [yAxisId](option.series-pictorialBar.md#yAxisId) 指定所使用的坐标轴。
...
也参见 [series-pictorialBar.coordinateSystemUsage](option.series-pictorialBar.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-pictorialBar.md#coordinateSystem) 上布局本 `series-pictorialBar`。
...
*   `'box'`：**（不适用于 [series-pictorialBar](option.series-pictorialBar.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-pictorialBar.coords](option.series-pictorialBar.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-pictorialBar.coordinateSystem](option.series-pictorialBar.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-pictorialBar.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.series-pictorialBar.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
淡出状态配置。开启 [emphasis.focus](option.series-pictorialBar.md#emphasis.focus) 后有效。
...
选中状态配置。开启 [selectedMode](option.series-pictorialBar.md#selectedMode) 后有效。
...
比 [barWidth](option.series-bar.md#barWidth) 优先级高。
...
此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbol)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbol)，表示只对此数据项生效。
...
当设置为百分比时，图形的大小是基于 [基准柱](option.series-pictorialBar.md) 的尺寸计算出来的。

例如，当基准柱基于 x 轴（即柱子是纵向的），[symbolSize](option.series-pictorialBar.md#symbolSize) 设置为 `['30%', '50%']`，那么最终图形的尺寸是：

*   宽度：`基准柱的宽度 * 30%`。
*   高度：
    *   如果 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat) 为 `false`：`基准柱的高度 * 50%`。
    *   如果 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat) 为 `true`：`基准柱的宽度 * 50%`。

基准柱基于 y 轴（即柱子是横向的）的情况类似对调可得出。

例子：

此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolSize)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolSize)，表示只对此数据项生效。
...
此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolPosition)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolPosition)，表示只对此数据项生效。
...
当设置为百分比时，表示相对于自身尺寸 [symbolSize](option.series-pictorialBar.md#symbolSize) 的百分比。

例如 `[0, '-50%']` 就是把图形向上移动了自身尺寸的一半的位置。

例子：

此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolOffset)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolOffset)，表示只对此数据项生效。
...
此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolRotate)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolRotate)，表示只对此数据项生效。
...
*   `true`：使图形元素重复，即每个数据值用一组重复的图形元素表示。重复的次数依据 [data](option.series-pictorialBar.md#data) 计算得到。
*   a number：使图形元素重复，即每个数据值用一组重复的图形元素表示。重复的次数是给定的定值。
*   `'fixed'`：使图形元素重复，即每个数据值用一组重复的图形元素表示。重复的次数依据 [symbolBoundingData](option.series-pictorialBar.md#symbolBoundingData) 计算得到，即与 [data](option.series-pictorialBar.md#data) 无关。这在此图形被用于做背景时有用。

例子：

此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolRepeat)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolRepeat)，表示只对此数据项生效。
...
*   当 [symbolMargin](option.series-pictorialBar.md#symbolMargin) 设置为负值时，重复的图形会互相覆盖，这是可以使用 `symbolRepeatDirection` 来指定覆盖顺序。
    
*   当 [animationDelay](option.series-pictorialBar.md#animationDelay) 或 [animationDelayUpdate](option.series-pictorialBar.md#animationDelayUpdate) 被使用时，`symbolRepeatDirection` 指定了 index 顺序。
...
此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolRepeatDirection)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolRepeatDirection)，表示只对此数据项生效。
...
图形的两边间隔（『两边』是指其数值轴方向的两边）。可以是绝对数值（如 `20`），或者百分比值（如 `'-30%'`），表示相对于自身尺寸 [symbolSize](option.series-pictorialBar.md#symbolSize) 的百分比。只有当 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat) 被使用时有意义。

可以是正值，表示间隔大；也可以是负数。当 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat) 被使用时，负数时能使图形重叠。

可以在其值结尾处加一个 `"!"`，如 `"30%!"` 或 `25!`，表示第一个图形的开始和最后一个图形结尾留白，不紧贴边界。默认会紧贴边界。

注意：

*   当 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat) 为 `true`/`'fixed'` 的时候： 这里设置的 `symbolMargin` 只是个参考值，真正最后的图形间隔，是根据 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat)、`symbolMargin`、[symbolBoundingData](option.series-pictorialBar.md#symbolBoundingData) 综合计算得到。
*   当 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat) 为一个固定数值的时候： 这里设置的 `symbolMargin` 无效。

例子：

此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolMargin)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolMargin)，表示只对此数据项生效。
...
*   『背景系列』和『当前值系列』使用相同的 [symbolBoundingData](../option.md#series.pictorialBar.symbolBoundingData)，使得绘制出的图形的大小是一样的。
*   『当前值系列』设置了比『背景系列』更高的 [z](../option.md#series.pictorialBar.z)，使得其覆盖在『背景系列』上。

此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolClip)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolClip)，表示只对此数据项生效。
...
*   没有使用 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat) 时：
...
*   使用了 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat) 时：
    
    `symbolBoundingData` 缺省情况取当前坐标系所显示出的最值。`symbolBoundingData` 定义了一个 bounding，重复的图形在这个 bounding 中，依据 [symbolMargin](option.series-pictorialBar.md#symbolMargin) 和 [symbolRepeat](option.series-pictorialBar.md#symbolRepeat) 和 [symbolSize](option.series-pictorialBar.md#symbolSize) 进行排布。这几个变量决定了图形的间隔大小。
    

在这些场景中，你可能会需要设置 `symbolBoundingData`：

*   使用了 [symbolCilp](option.series-pictorialBar.md#symbolClip) 时：
...
此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolBoundingData)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolBoundingData)，表示只对此数据项生效。
...
此属性可以被设置在系列的 [根部](option.series-pictorialBar.md#symbolPatternSize)，表示对此系列中所有数据都生效；也可以被设置在 [data](series-pictorialBar.data) 中的 [每个数据项中](option.series-pictorialBar.md#data.symbolPatternSize)，表示只对此数据项生效。
...
柱条的颜色。 在`emphasis`状态中支持设置为 `'inherit'` 取消高亮。
...
其中第一个参数 `value` 为 [data](option.series-pictorialBar.md#data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1.  通过 [x](option.series-pictorialBar.md#markPoint.data.x), [y](option.series-pictorialBar.md#markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-pictorialBar.md#markPoint.data.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](option.series-pictorialBar.md#markPoint.data.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](option.series-pictorialBar.md#markPoint.data.valueIndex) 或者 [valueDim](option.series-pictorialBar.md#markPoint.data.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
标注名称。定义后可在 [label formatter](option.series-pictorialBar.md#markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
在使用 [type](option.series-pictorialBar.md#markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

#### markPoint.data.valueDim
- **Type**: `string`

在使用 [type](option.series-pictorialBar.md#markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](option.series-pictorialBar.md#markPoint.data.x) 和 [y](option.series-pictorialBar.md#markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](option.series-pictorialBar.md#markLine.data.0.symbol)。
...
1.  通过 [x](option.series-pictorialBar.md#markLine.data.0.x), [y](option.series-pictorialBar.md#markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-pictorialBar.md#markLine.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](option.series-pictorialBar.md#markLine.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](option.series-pictorialBar.md#markLine.data.0.valueIndex) 或者 [valueDim](option.series-pictorialBar.md#markLine.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](option.series-pictorialBar.md#markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markLine.data.0.valueDim
- **Type**: `string`

在使用 [type](option.series-pictorialBar.md#markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称。定义后可在 [label formatter](option.series-pictorialBar.md#markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
1.  通过 [x](option.series-pictorialBar.md#markArea.data.0.x), [y](option.series-pictorialBar.md#markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-pictorialBar.md#markArea.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](option.series-pictorialBar.md#markArea.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](option.series-pictorialBar.md#markArea.data.0.valueIndex) 或者 [valueDim](option.series-pictorialBar.md#markPoint.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](option.series-pictorialBar.md#markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markArea.data.0.valueDim
- **Type**: `string`

在使用 [type](option.series-pictorialBar.md#markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](option.series-pictorialBar.md#markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
自 `v6.1.0`，任一柱子超出坐标系的部分会被剪裁掉。 `v6.1.0` 前（不含），任一柱子如果中心超出坐标系则被整体剪裁掉，否则整体保留哪怕部分超出。
...
象形柱图组件的所有图形的`z`值。控制图形的前后顺序。`z`值小的图形会被`z`值大的图形覆盖。
