...
指定另一个坐标系组件，本 `series-heatmap` 布局在那个坐标系中。

可选值：

*   `'cartesian2d'`
    
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](option.grid.md) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](option.series-heatmap.md#xAxisIndex) 和 [yAxisIndex](option.series-heatmap.md#yAxisIndex) 或者 [xAxisId](option.series-heatmap.md#xAxisId) 和 [yAxisId](option.series-heatmap.md#yAxisId) 指定所使用的坐标轴。
...
*   `'geo'`
    ...
    布局在一个 [地理坐标系](option.geo.md) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](option.series-heatmap.md#geoIndex) 或 [geoId](option.series-heatmap.md#geoId) 指定所使用的地理坐标系。
    

*   `'calendar'`
    
    布局在一个 [日历坐标系](option.calendar.md) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](option.series-heatmap.md#calendarIndex) 或 [calendarId](option.series-heatmap.md#calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](option.matrix.md)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](option.series-heatmap.md#matrixIndex) 或 [matrixId](option.series-heatmap.md#matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-heatmap.coordinateSystemUsage](option.series-heatmap.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-heatmap.md#coordinateSystem) 上布局本 `series-heatmap`。
...
*   `'box'`：**（不适用于 [series-heatmap](option.series-heatmap.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-heatmap.coords](option.series-heatmap.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-heatmap.coordinateSystem](option.series-heatmap.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-heatmap.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.series-heatmap.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
每个点的大小，在地理坐标系([coordinateSystem](option.series-heatmap.md#coordinateSystem): 'geo')上有效。

## blurSize
- **Type**: `number`
- **Default**: `20`

每个点模糊的大小，在地理坐标系([coordinateSystem](option.series-heatmap.md#coordinateSystem): 'geo')上有效。

## minOpacity
- **Type**: `number`
- **Default**: `0`

最小的透明度，在地理坐标系([coordinateSystem](option.series-heatmap.md#coordinateSystem): 'geo')上有效。

## maxOpacity
- **Type**: `number`
- **Default**: `1`

最大的透明度，在地理坐标系([coordinateSystem](option.series-heatmap.md#coordinateSystem): 'geo')上有效。
...
在直角坐标系([coordinateSystem](option.series-heatmap.md#coordinateSystem): 'cartesian2d')上有效。
...
样式设置，在直角坐标系([coordinateSystem](option.series-heatmap.md#coordinateSystem): 'cartesian2d')上有效。
...
淡出状态的图形样式。开启 [emphasis.focus](option.series-heatmap.md#emphasis.focus) 后有效。
...
选中状态的图形样式。开启 [selectedMode](option.series-heatmap.md#selectedMode) 后有效。
...
单个数据点的样式设置，在直角坐标系([coordinateSystem](option.series-heatmap.md#coordinateSystem): 'cartesian2d')上有效。
...
其中第一个参数 `value` 为 [data](option.series-heatmap.md#data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1.  通过 [x](option.series-heatmap.md#markPoint.data.x), [y](option.series-heatmap.md#markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称。定义后可在 [label formatter](option.series-heatmap.md#markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](option.series-heatmap.md#markPoint.data.x) 和 [y](option.series-heatmap.md#markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](option.series-heatmap.md#markLine.data.0.symbol)。
...
1.  通过 [x](option.series-heatmap.md#markLine.data.0.x), [y](option.series-heatmap.md#markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称。定义后可在 [label formatter](option.series-heatmap.md#markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
1.  通过 [x](option.series-heatmap.md#markArea.data.0.x), [y](option.series-heatmap.md#markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](option.series-heatmap.md#markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
热力图组件的所有图形的`z`值。控制图形的前后顺序。`z`值小的图形会被`z`值大的图形覆盖。
