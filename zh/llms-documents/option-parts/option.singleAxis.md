...
也参见 [boundaryGap](option.singleAxis.md#boundaryGap)。这两个配置项的功能因为历史原因有所重叠。只有在 `boundaryGap: false, containShape: false` 时，类目坐标轴（`axis.type: 'category'`）中的图形才可能超出坐标轴范围。
...
只在数值轴、对数轴、时间轴中（[type](option.singleAxis.md#type): 'value'、'log' 或 'time'）有效。
...
**与 [min](option.singleAxis.md#min) 的区别：**
...
**与 [max](option.singleAxis.md#max) 的区别：**
