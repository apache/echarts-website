...
需要配合其他配置项共同实现动态排序柱状图效果，具体参见[动态排序柱状图](https://echarts.apache.org/handbook/zh/how-to/chart-types/bar/bar-race)教程。
...
*   `'category'` 类目轴，适用于离散的类目数据。为该类型时类目数据可自动从 [series.data](../option.md#series.data) 或 [dataset.source](option.dataset.md#source) 中取，或者可通过 [yAxis.data](option.yAxis.md#data) 设置类目数据。
...
坐标轴名称的颜色，默认取 [axisLine.lineStyle.color](option.yAxis.md#axisLine.lineStyle.color)。
...
类目轴中 `boundaryGap` 可以配置为 `true` 和 `false`。默认为 `true`，这时候[刻度](option.yAxis.md#axisTick)只是作为分隔线，标签和数据点都会在两个[刻度](option.yAxis.md#axisTick)之间的带(band)中间。

非类目轴，包括时间，数值，对数轴，`boundaryGap`是一个两个值的数组，分别表示数据最小值和最大值的延伸范围，可以直接设置数值或者相对的百分比，在设置 [min](option.yAxis.md#min) 和 [max](option.yAxis.md#max) 后无效。 **示例：**
...
也参见 [boundaryGap](option.yAxis.md#boundaryGap)。这两个配置项的功能因为历史原因有所重叠。只有在 `boundaryGap: false, containShape: false` 时，类目坐标轴（`axis.type: 'category'`）中的图形才可能超出坐标轴范围。
...
只在数值轴、对数轴、时间轴中（[type](option.yAxis.md#type): 'value'、'log' 或 'time'）有效。
...
只在数值轴中（[type](option.yAxis.md#type): 'value'）有效。

是否是脱离 0 值比例。设置成 `true` 后坐标刻度不会强制包含零刻度。在双数值轴的散点图中比较有用。

在设置 [min](option.yAxis.md#min) 和 [max](option.yAxis.md#max) 之后该配置项无效。
...
只在数值轴或时间轴中（[type](option.yAxis.md#type): 'value' 或 'time'）有效。
...
例如，在时间轴（（[type](option.yAxis.md#type): 'time'））可以设置成 `3600 * 24 * 1000` 保证坐标轴分割刻度最大为一天。
...
因为 [splitNumber](option.yAxis.md#splitNumber) 是预估的值，实际根据策略计算出来的刻度可能无法达到想要的效果，这时候可以使用 interval 配合 [min](option.yAxis.md#min)、[max](option.yAxis.md#max) 强制设定刻度划分，一般不建议使用。

无法在类目轴中使用。在时间轴（[type](option.yAxis.md#type): 'time'）中需要传时间戳，在对数轴（[type](option.yAxis.md#type): 'log'）中需要传指数值。

## logBase
- **Type**: `number`
- **Default**: `10`

对数轴的底数，只在对数轴中（[type](option.yAxis.md#type): 'log'）有效。
...
是否允许 [jitter](option.yAxis.md#jitter) 重叠。如果设为 `false`，将尽可能避免重叠，极端情况下如果重叠是无法避免的，则也可能出现重叠。以下是设为 `false` 的效果：
...
在设置了 [jitter](option.yAxis.md#jitter) 且 [jitterOverlap](option.yAxis.md#jitterOverlap) 为 `false` 的情况下，两个数据点之间的最小距离。
...
**其他例子：** [bar-breaks-simple](https://echarts.apache.org/examples/zh/editor.html?c=bar-breaks-simple&edit=1&reset=1), [line-fisheye-lens](https://echarts.apache.org/examples/zh/editor.html?c=line-fisheye-lens&edit=1&reset=1)
...
> 
> 断轴无法在类目轴（[type](option.yAxis.md#type): `'category'`）中使用。

如果 ECharts 的 `import` 方式是 [只 `import` 所需要的组件](https://echarts.apache.org/handbook/zh/basics/import)，断轴功能需要被手动 `import` 和注册。例如，
...
*   如果 [axis.type](option.yAxis.md#type) 是 `'value'` 或 `'log'`，则使用 `number` 类型的值。
    
*   如果 [axis.type](option.yAxis.md#type) 是 `'category'`：不支持。
    
*   如果 [axis.type](option.yAxis.md#type) 是 `'time'`，值可以是：
    
    *   `string` 类型，表示任意能被 [方法 `parseDate` (`echarts/src/util/number.ts`)](https://github.com/apache/echarts/blob/master/src/util/number.ts) 解析的时间格式，例如 `'2024-04-09 13:00:00'`。
    *   `number` 类型，表示时间戳，例如 `1712667600000`。
    *   `Date` 类型的时间对象，例如 `new Date('2024-04-09T13:00:00Z')`。

注：[yAxis.breaks.start](option.yAxis.md#breaks.start) 和 [yAxis.breaks.end](option.yAxis.md#breaks.end) 是每个 break 项的唯一标志。当调用 [chart.setOption](../api-parts/api.echartsInstance.md#setOption) 修改 [yAxis.breaks.gap](option.yAxis.md#breaks.gap) 或 [yAxis.breaks.isExpanded](option.yAxis.md#breaks.isExpanded) 时，`start` `end` 必须指定，且如果 `start` `end` 不修改才会有更新动画，修改了则无。
...
*   百分比（字符串）：
    ...
    表示相对于坐标轴的比例。例如 `'5%'`，表示断轴截断区域最终显示的尺寸总为坐标轴尺寸的 `'5%'`。使用百分比能保证断轴截断区域的像素尺寸稳定，不随着 [yAxis.min](option.yAxis.md#min)、[yAxis.max](option.yAxis.md#max)、[dataZoom](../option.md#dataZoom) 变化而变化，因此目前大多数场景都适合使用百分比。
    
*   绝对值：
    
    其单位和 [start](option.yAxis.md#breaks.start) 与 [end](option.yAxis.md#breaks.end) 相同，是业务数据（`series.data`）所定义的值域中的某个值，而非像素值。它表示把 `[start, end]` 这个区间映射（替换）为 `[start, start + gap]`。因此，设为绝对值时，断轴截断区域的像素尺寸会随着 [yAxis.min](option.yAxis.md#min)、[yAxis.max](option.yAxis.md#max)、[dataZoom](../option.md#dataZoom) 变化而变化。
    

**注意：**在一个 [yAxis.breaks](option.yAxis.md#breaks) 数组中，`gap` 只允许全使用百分比，或者全使用绝对值，不允许混合使用，否者效果可能不符合预期。
...
断轴的基本介绍参见 [yAxis.breaks](../option.md#yAxisbreaks)。
...
X 轴或者 Y 轴的轴线是否在另一个轴的 0 刻度上。只有在另一个轴为 [数值轴（'value'）](option.xAxis.md#type) 且包含 0 刻度时有效。
...
坐标轴刻度的显示间隔，在类目轴中有效。默认同 [axisLabel.interval](option.yAxis.md#axisLabel.interval) 一样。
...
刻度线的颜色，默认取 [axisLine.lineStyle.color](option.yAxis.md#axisLine.lineStyle.color)。
...
注意：次刻度线无法在类目轴（[type](option.yAxis.md#type): `'category'`）中使用。
...
**对于时间轴（[`yAxis.type: 'time'`](option.yAxis.md#type)）**
...
最小的坐标刻度标签的对齐方式。如果设置为 `null`，则和其他标签一致。参见 [align](option.yAxis.md#axisLabel.verticalAlign)。
...
最大的坐标刻度标签的对齐方式。如果设置为 `null`，则和其他标签一致。参见 [align](option.yAxis.md#axisLabel.verticalAlign)。
...
刻度标签文字的颜色，默认取 [axisLine.lineStyle.color](option.yAxis.md#axisLine.lineStyle.color)。支持回调函数，格式如下
...
坐标轴分隔线的显示间隔，在类目轴中有效。默认同 [axisLabel.interval](option.yAxis.md#axisLabel.interval) 一样。
...
坐标轴在 [grid](option.grid.md) 区域中的次分隔线。次分割线会对齐次刻度线 [minorTick](option.yAxis.md#minorTick)
...
坐标轴分隔区域的显示间隔，在类目轴中有效。默认同 [axisLabel.interval](option.yAxis.md#axisLabel.interval) 一样。
...
类目数据，在类目轴（[type](option.yAxis.md#type): `'category'`）中有效。

如果没有设置 [type](option.yAxis.md#type)，但是设置了 `axis.data`，则认为 `type` 是 `'category'`。

如果设置了 [type](option.yAxis.md#type) 是 `'category'`，但没有设置 `axis.data`，则 `axis.data` 的内容会自动从 [series.data](../option.md#series.data) 中获取，这会比较方便。不过注意，`axis.data` 指明的是 `'category'` 轴的取值范围。如果不指定而是从 [series.data](../option.md#series.data) 中获取，那么只能获取到 [series.data](../option.md#series.data) 中出现的值。比如说，假如 [series.data](../option.md#series.data) 为空时，就什么也获取不到。
...
坐标轴 tooltip 设置，注意需设置 [triggerEvent](option.yAxis.md#triggerEvent) 为 `true` 并启用全局 [option.tooltip](option.tooltip.md) 组件。
...
Y 轴组件的所有图形的`z`值。控制图形的前后顺序。`z`值小的图形会被`z`值大的图形覆盖。
