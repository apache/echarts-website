...
指定另一个坐标系组件，本 `series-graph` 布局在那个坐标系中。
...
*   `'cartesian2d'`
    ...
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](option.grid.md) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](option.series-graph.md#xAxisIndex) 和 [yAxisIndex](option.series-graph.md#yAxisIndex) 或者 [xAxisId](option.series-graph.md#xAxisId) 和 [yAxisId](option.series-graph.md#yAxisId) 指定所使用的坐标轴。
...
*   `'polar'`
    ...
    布局在一个 [极坐标系](option.polar.md) 中。当一个 ECharts 实例中存在多个极坐标系时，须通过 [polarIndex](option.series-graph.md#polarIndex) 或 [polarId](option.series-graph.md#polarId) 指定所使用的极坐标系。
    

*   `'geo'`
    
    布局在一个 [地理坐标系](option.geo.md) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](option.series-graph.md#geoIndex) 或 [geoId](option.series-graph.md#geoId) 指定所使用的地理坐标系。
    

*   `'singleAxis'`
    
    布局在一个 [单轴坐标系](option.singleAxis.md) 中。当一个 ECharts 实例中存在多个单轴坐标系时，须通过 [singleAxisIndex](option.series-graph.md#singleAxisIndex) 或 [singleAxisId](option.series-graph.md#singleAxisId) 指定所使用的单轴标系。
    

*   `'calendar'`
    
    布局在一个 [日历坐标系](option.calendar.md) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](option.series-graph.md#calendarIndex) 或 [calendarId](option.series-graph.md#calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](option.matrix.md)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](option.series-graph.md#matrixIndex) 或 [matrixId](option.series-graph.md#matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-graph.coordinateSystemUsage](option.series-graph.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-graph.md#coordinateSystem) 上布局本 `series-graph`。
...
*   `'box'`：**（不适用于 [series-graph](option.series-graph.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-graph.coords](option.series-graph.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-graph.coordinateSystem](option.series-graph.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-graph.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.series-graph.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
*   `'none'` 不采用任何布局，使用[节点](option.series-graph.md#data)中提供的 [x](option.series-graph.md#data.x)， [y](option.series-graph.md#data.y) 作为节点的位置。
    
*   `'circular'` 采用环形布局，见示例 [Les Miserables](https://echarts.apache.org/examples/zh/editor.html?c=graph-circular-layout)，布局相关的配置项见 [graph.circular](option.series-graph.md#circular)
    
*   `'force'` 采用力引导布局，见示例 [Force](https://echarts.apache.org/examples/zh/editor.html?c=graph-force)，布局相关的配置项见 [graph.force](option.series-graph.md#force)
...
默认不进行任何布局，使用[节点](option.series-graph.md#data)中提供的 [x](option.series-graph.md#data.x)， [y](option.series-graph.md#data.y) 作为节点的位置。如果不存在的话会随机生成一个位置。
...
边的两个节点之间的距离，这个距离也会受 [repulsion](option.series-graph.md#force.repulsion)。
...
`center` 一般用于控制或获取 [平移缩放（roam）](option.series-graph.md#roam) 时图形的位置。当平移缩放时，`center` 和 [zoom](option.series-graph.md#zoom) 的值会被相应改变。

注意：`center` 的值基于的是对象布局的原始坐标系而非画布的坐标。 如果想使用画布坐标调整绘制对象的位置和大小，使用：[series-graph.left](option.series-graph.md#left) / [.right](option.series-graph.md#right) / [.top](option.series-graph.md#top) / [.bottom](option.series-graph.md#bottom) / [.width](option.series-graph.md#width) / [.height](option.series-graph.md#height) 。

如果 `center` 中使用绝对数值：

*   如果 [series-graph.layout](option.series-graph.md#layout) 为 `'none'`，你需要在 [series-graph.data.x](option.series-graph.md#data.x), [series-graph.data.y](option.series-graph.md#data.y) 里提供每个节点的坐标。这时 `center` 中可使用绝对数值表示这种坐标。例如，
...
*   否则，如果在 [series-graph.layout](option.series-graph.md#layout) 中设定了自动布局，坐标是自动计算的不可提前预知，于是也无法用绝对数值在 `center` 中设定合适的坐标。
...
**参考示例**来理解此概念：[graph roam indicator](https://echarts.apache.org/examples/zh/editor.html?c=doc-example/graph-roam-indicator&edit=1&reset=1).
...
当 [平移缩放（roam）](option.series-graph.md#roam) 时，[center](option.series-graph.md#center) 和 `zoom` 的值会被相应改变。
...
[缩放](option.series-graph.md#roam) 的极限控制，通过 `min` 和 `max` 限制最小和最大的缩放值。
...
当缩放和平移时，[center](option.series-graph.md#center) 和 [zoom](option.series-graph.md#zoom) 的值会被相应改变。
...
[缩放和平移（roam）](option.series-graph.md#roam) 可被鼠标触发。
...
注意：`v5.4.1` 之前的版本只在使用[力引导布局](option.series-graph.md#force)的时候才有用。
...
节点标记的大小，可以设置成诸如 `10` 这样单一的数字，也可以用数组分开表示宽和高，例如 `[20, 10]` 表示标记宽为`20`，高为`10`。
...
关系边的公用线条样式。其中 [lineStyle.color](option.series-graph.md#lineStyle.color) 支持设置为`'source'`或者`'target'`特殊值，此时边会自动取源节点或目标节点的颜色作为自己的颜色。
...
淡出状态的图形样式。开启 [emphasis.focus](option.series-graph.md#emphasis.focus) 后有效。
...
选中状态的图形样式。开启 [selectedMode](option.series-graph.md#selectedMode) 后有效。
...
如果节点有分类的话可以通过 [data\[i\].category](option.series-graph.md#data.category) 指定每个节点的类目，类目的样式会被应用到节点样式上。[图例](option.legend.md)也可以基于`categories`名字展现和筛选。

### categories.name
- **Type**: `string`

类目名称，用于和 [legend](option.legend.md) 对应以及格式化 [tooltip](option.tooltip.md) 的内容。
...
#### categories.itemStyle.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
#### categories.itemStyle.borderCap
- **Type**: `string`
- **Default**: `'butt'`
...
#### categories.itemStyle.borderJoin
- **Type**: `string`
- **Default**: `'bevel'`
...
#### categories.itemStyle.borderMiterLimit
- **Type**: `number`
- **Default**: `10`
...
#### categories.label.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
#### categories.label.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
#### categories.label.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### categories.label.rich.<style_name>.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### categories.label.rich.<style_name>.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### categories.label.rich.<style_name>.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### categories.label.rich.<style_name>.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
#### categories.label.richInheritPlainLabel
- **Type**: `boolean`
- **Default**: `true`
...
##### categories.emphasis.itemStyle.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.emphasis.itemStyle.borderCap
- **Type**: `string`
- **Default**: `'butt'`
...
##### categories.emphasis.itemStyle.borderJoin
- **Type**: `string`
- **Default**: `'bevel'`
...
##### categories.emphasis.itemStyle.borderMiterLimit
- **Type**: `number`
- **Default**: `10`
...
##### categories.emphasis.label.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### categories.emphasis.label.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.emphasis.label.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### categories.emphasis.label.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.emphasis.label.overflow
- **Type**: `string`
- **Default**: `'none'`
...
##### categories.emphasis.label.ellipsis
- **Type**: `string`
- **Default**: `'...'`
...
###### categories.emphasis.label.rich.<style_name>.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### categories.emphasis.label.rich.<style_name>.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### categories.emphasis.label.rich.<style_name>.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### categories.emphasis.label.rich.<style_name>.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.emphasis.label.richInheritPlainLabel
- **Type**: `boolean`
- **Default**: `true`
...
##### categories.blur.itemStyle.color
- **Type**: `Color`
- **Default**: `自适应`
...
##### categories.blur.itemStyle.borderColor
- **Type**: `Color`
- **Default**: `'#000'`

图形的描边颜色。支持的颜色格式同 `color`，不支持回调函数。

##### categories.blur.itemStyle.borderWidth
- **Type**: `number`
- **Default**: `0`

描边线宽。为 0 时无描边。

##### categories.blur.itemStyle.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### categories.blur.itemStyle.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.blur.itemStyle.borderCap
- **Type**: `string`
- **Default**: `'butt'`
...
##### categories.blur.itemStyle.borderJoin
- **Type**: `string`
- **Default**: `'bevel'`
...
##### categories.blur.itemStyle.borderMiterLimit
- **Type**: `number`
- **Default**: `10`
...
##### categories.blur.itemStyle.shadowBlur
- **Type**: `number`
...
##### categories.blur.itemStyle.shadowColor
- **Type**: `Color`

阴影颜色。支持的格式同`color`。

##### categories.blur.itemStyle.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

阴影水平方向上的偏移距离。

##### categories.blur.itemStyle.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

阴影垂直方向上的偏移距离。

##### categories.blur.itemStyle.opacity
- **Type**: `number`
- **Default**: `1`

图形透明度。支持从 0 到 1 的数字，为 0 时不绘制该图形。

##### categories.blur.label.show
- **Type**: `boolean`
- **Default**: `false`

是否显示标签。

##### categories.blur.label.position
- **Type**: `string|Array`
...
##### categories.blur.label.distance
- **Type**: `number`
- **Default**: `5`
...
##### categories.blur.label.offset
- **Type**: `Array`

是否对文字进行偏移。默认不偏移。例如：`[30, 40]` 表示文字在横向上偏移 `30`，纵向上偏移 `40`。

##### categories.blur.label.color
- **Type**: `Color`
- **Default**: `'#fff'`
...
##### categories.blur.label.fontStyle
- **Type**: `string`
- **Default**: `'normal'`
...
##### categories.blur.label.fontWeight
- **Type**: `string|number`
- **Default**: `'normal'`
...
##### categories.blur.label.fontFamily
- **Type**: `string`
- **Default**: `'sans-serif'`
...
##### categories.blur.label.fontSize
- **Type**: `number`
- **Default**: `12`

文字的字体大小。

##### categories.blur.label.align
- **Type**: `string`
...
##### categories.blur.label.verticalAlign
- **Type**: `string`
...
##### categories.blur.label.lineHeight
- **Type**: `number`
...
##### categories.blur.label.backgroundColor
- **Type**: `string|Object`
- **Default**: `'transparent'`
...
##### categories.blur.label.borderColor
- **Type**: `Color`
...
##### categories.blur.label.borderWidth
- **Type**: `number`
- **Default**: `0`

文字块边框宽度。

##### categories.blur.label.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### categories.blur.label.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.blur.label.borderRadius
- **Type**: `number|Array`
- **Default**: `0`

文字块的圆角。

##### categories.blur.label.padding
- **Type**: `number|Array`
- **Default**: `0`
...
##### categories.blur.label.shadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字块的背景阴影颜色。

##### categories.blur.label.shadowBlur
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影长度。

##### categories.blur.label.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 X 偏移。

##### categories.blur.label.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 Y 偏移。

##### categories.blur.label.width
- **Type**: `number`

文本显示宽度。

##### categories.blur.label.height
- **Type**: `number`

文本显示高度。

##### categories.blur.label.textBorderColor
- **Type**: `Color`
...
##### categories.blur.label.textBorderWidth
- **Type**: `number`

文字本身的描边宽度。

##### categories.blur.label.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### categories.blur.label.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.blur.label.textShadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字本身的阴影颜色。

##### categories.blur.label.textShadowBlur
- **Type**: `number`
- **Default**: `0`

文字本身的阴影长度。

##### categories.blur.label.textShadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 X 偏移。

##### categories.blur.label.textShadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 Y 偏移。

##### categories.blur.label.overflow
- **Type**: `string`
- **Default**: `'none'`
...
##### categories.blur.label.ellipsis
- **Type**: `string`
- **Default**: `'...'`

在`overflow`配置为`'truncate'`的时候，可以通过该属性配置末尾显示的文本。

##### categories.blur.label.rich
- **Type**: `Object`
...
###### categories.blur.label.rich.<style_name>.color
- **Type**: `Color`
- **Default**: `'#fff'`
...
###### categories.blur.label.rich.<style_name>.fontStyle
- **Type**: `string`
- **Default**: `'normal'`
...
###### categories.blur.label.rich.<style_name>.fontWeight
- **Type**: `string|number`
- **Default**: `'normal'`
...
###### categories.blur.label.rich.<style_name>.fontFamily
- **Type**: `string`
- **Default**: `'sans-serif'`
...
###### categories.blur.label.rich.<style_name>.fontSize
- **Type**: `number`
- **Default**: `12`

文字的字体大小。

###### categories.blur.label.rich.<style_name>.align
- **Type**: `string`
...
###### categories.blur.label.rich.<style_name>.verticalAlign
- **Type**: `string`
...
###### categories.blur.label.rich.<style_name>.lineHeight
- **Type**: `number`
...
###### categories.blur.label.rich.<style_name>.backgroundColor
- **Type**: `string|Object`
- **Default**: `'transparent'`
...
###### categories.blur.label.rich.<style_name>.borderColor
- **Type**: `Color`
...
###### categories.blur.label.rich.<style_name>.borderWidth
- **Type**: `number`
- **Default**: `0`

文字块边框宽度。

###### categories.blur.label.rich.<style_name>.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### categories.blur.label.rich.<style_name>.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### categories.blur.label.rich.<style_name>.borderRadius
- **Type**: `number|Array`
- **Default**: `0`

文字块的圆角。

###### categories.blur.label.rich.<style_name>.padding
- **Type**: `number|Array`
- **Default**: `0`
...
###### categories.blur.label.rich.<style_name>.shadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字块的背景阴影颜色。

###### categories.blur.label.rich.<style_name>.shadowBlur
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影长度。

###### categories.blur.label.rich.<style_name>.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 X 偏移。

###### categories.blur.label.rich.<style_name>.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 Y 偏移。

###### categories.blur.label.rich.<style_name>.width
- **Type**: `number|string`
...
###### categories.blur.label.rich.<style_name>.height
- **Type**: `number|string`
...
###### categories.blur.label.rich.<style_name>.textBorderColor
- **Type**: `Color`
...
###### categories.blur.label.rich.<style_name>.textBorderWidth
- **Type**: `number`

文字本身的描边宽度。

###### categories.blur.label.rich.<style_name>.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### categories.blur.label.rich.<style_name>.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### categories.blur.label.rich.<style_name>.textShadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字本身的阴影颜色。

###### categories.blur.label.rich.<style_name>.textShadowBlur
- **Type**: `number`
- **Default**: `0`

文字本身的阴影长度。

###### categories.blur.label.rich.<style_name>.textShadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 X 偏移。

###### categories.blur.label.rich.<style_name>.textShadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 Y 偏移。

##### categories.blur.label.richInheritPlainLabel
- **Type**: `boolean`
- **Default**: `true`
...
##### categories.select.itemStyle.color
- **Type**: `Color`
- **Default**: `自适应`
...
##### categories.select.itemStyle.borderColor
- **Type**: `Color`
- **Default**: `'#000'`

图形的描边颜色。支持的颜色格式同 `color`，不支持回调函数。

##### categories.select.itemStyle.borderWidth
- **Type**: `number`
- **Default**: `0`

描边线宽。为 0 时无描边。

##### categories.select.itemStyle.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### categories.select.itemStyle.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.select.itemStyle.borderCap
- **Type**: `string`
- **Default**: `'butt'`
...
##### categories.select.itemStyle.borderJoin
- **Type**: `string`
- **Default**: `'bevel'`
...
##### categories.select.itemStyle.borderMiterLimit
- **Type**: `number`
- **Default**: `10`
...
##### categories.select.itemStyle.shadowBlur
- **Type**: `number`
...
##### categories.select.itemStyle.shadowColor
- **Type**: `Color`

阴影颜色。支持的格式同`color`。

##### categories.select.itemStyle.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

阴影水平方向上的偏移距离。

##### categories.select.itemStyle.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

阴影垂直方向上的偏移距离。

##### categories.select.itemStyle.opacity
- **Type**: `number`
- **Default**: `1`

图形透明度。支持从 0 到 1 的数字，为 0 时不绘制该图形。

##### categories.select.label.show
- **Type**: `boolean`
- **Default**: `false`

是否显示标签。

##### categories.select.label.position
- **Type**: `string|Array`
...
##### categories.select.label.distance
- **Type**: `number`
- **Default**: `5`
...
##### categories.select.label.rotate
- **Type**: `number`
...
##### categories.select.label.offset
- **Type**: `Array`

是否对文字进行偏移。默认不偏移。例如：`[30, 40]` 表示文字在横向上偏移 `30`，纵向上偏移 `40`。

##### categories.select.label.color
- **Type**: `Color`
- **Default**: `'#fff'`
...
##### categories.select.label.fontStyle
- **Type**: `string`
- **Default**: `'normal'`
...
##### categories.select.label.fontWeight
- **Type**: `string|number`
- **Default**: `'normal'`
...
##### categories.select.label.fontFamily
- **Type**: `string`
- **Default**: `'sans-serif'`
...
##### categories.select.label.fontSize
- **Type**: `number`
- **Default**: `12`

文字的字体大小。

##### categories.select.label.align
- **Type**: `string`
...
##### categories.select.label.verticalAlign
- **Type**: `string`
...
##### categories.select.label.lineHeight
- **Type**: `number`
...
##### categories.select.label.backgroundColor
- **Type**: `string|Object`
- **Default**: `'transparent'`
...
##### categories.select.label.borderColor
- **Type**: `Color`
...
##### categories.select.label.borderWidth
- **Type**: `number`
- **Default**: `0`

文字块边框宽度。

##### categories.select.label.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### categories.select.label.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.select.label.borderRadius
- **Type**: `number|Array`
- **Default**: `0`

文字块的圆角。

##### categories.select.label.padding
- **Type**: `number|Array`
- **Default**: `0`
...
##### categories.select.label.shadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字块的背景阴影颜色。

##### categories.select.label.shadowBlur
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影长度。

##### categories.select.label.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 X 偏移。

##### categories.select.label.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 Y 偏移。

##### categories.select.label.width
- **Type**: `number`

文本显示宽度。

##### categories.select.label.height
- **Type**: `number`

文本显示高度。

##### categories.select.label.textBorderColor
- **Type**: `Color`
...
##### categories.select.label.textBorderWidth
- **Type**: `number`

文字本身的描边宽度。

##### categories.select.label.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### categories.select.label.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### categories.select.label.textShadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字本身的阴影颜色。

##### categories.select.label.textShadowBlur
- **Type**: `number`
- **Default**: `0`

文字本身的阴影长度。

##### categories.select.label.textShadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 X 偏移。

##### categories.select.label.textShadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 Y 偏移。

##### categories.select.label.overflow
- **Type**: `string`
- **Default**: `'none'`
...
##### categories.select.label.ellipsis
- **Type**: `string`
- **Default**: `'...'`

在`overflow`配置为`'truncate'`的时候，可以通过该属性配置末尾显示的文本。

##### categories.select.label.rich
- **Type**: `Object`
...
###### categories.select.label.rich.<style_name>.color
- **Type**: `Color`
- **Default**: `'#fff'`
...
###### categories.select.label.rich.<style_name>.fontStyle
- **Type**: `string`
- **Default**: `'normal'`
...
###### categories.select.label.rich.<style_name>.fontWeight
- **Type**: `string|number`
- **Default**: `'normal'`
...
###### categories.select.label.rich.<style_name>.fontFamily
- **Type**: `string`
- **Default**: `'sans-serif'`
...
###### categories.select.label.rich.<style_name>.fontSize
- **Type**: `number`
- **Default**: `12`

文字的字体大小。

###### categories.select.label.rich.<style_name>.align
- **Type**: `string`
...
###### categories.select.label.rich.<style_name>.verticalAlign
- **Type**: `string`
...
###### categories.select.label.rich.<style_name>.lineHeight
- **Type**: `number`
...
###### categories.select.label.rich.<style_name>.backgroundColor
- **Type**: `string|Object`
- **Default**: `'transparent'`
...
###### categories.select.label.rich.<style_name>.borderColor
- **Type**: `Color`
...
###### categories.select.label.rich.<style_name>.borderWidth
- **Type**: `number`
- **Default**: `0`

文字块边框宽度。

###### categories.select.label.rich.<style_name>.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### categories.select.label.rich.<style_name>.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### categories.select.label.rich.<style_name>.borderRadius
- **Type**: `number|Array`
- **Default**: `0`

文字块的圆角。

###### categories.select.label.rich.<style_name>.padding
- **Type**: `number|Array`
- **Default**: `0`
...
###### categories.select.label.rich.<style_name>.shadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字块的背景阴影颜色。

###### categories.select.label.rich.<style_name>.shadowBlur
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影长度。

###### categories.select.label.rich.<style_name>.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 X 偏移。

###### categories.select.label.rich.<style_name>.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 Y 偏移。

###### categories.select.label.rich.<style_name>.width
- **Type**: `number|string`
...
###### categories.select.label.rich.<style_name>.height
- **Type**: `number|string`
...
###### categories.select.label.rich.<style_name>.textBorderColor
- **Type**: `Color`
...
###### categories.select.label.rich.<style_name>.textBorderWidth
- **Type**: `number`

文字本身的描边宽度。

###### categories.select.label.rich.<style_name>.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### categories.select.label.rich.<style_name>.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### categories.select.label.rich.<style_name>.textShadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字本身的阴影颜色。

###### categories.select.label.rich.<style_name>.textShadowBlur
- **Type**: `number`
- **Default**: `0`

文字本身的阴影长度。

###### categories.select.label.rich.<style_name>.textShadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 X 偏移。

###### categories.select.label.rich.<style_name>.textShadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 Y 偏移。

##### categories.select.label.richInheritPlainLabel
- **Type**: `boolean`
- **Default**: `true`
...
**注意：** 如果设置 [lineStyle.curveness](option.series-graph.md#lineStyle.curveness) 则此属性失效。
...
别名，同 [data](option.series-graph.md#data)
...
边的[源节点名称](option.series-graph.md#data.name)的字符串，也支持使用数字表示源节点的索引。

### links.target
- **Type**: `string|number`

边的[目标节点名称](option.series-graph.md#data.name)的字符串，也支持使用数字表示源节点的索引。
...
#### links.label.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### links.label.rich.<style_name>.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### links.label.rich.<style_name>.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### links.label.rich.<style_name>.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### links.label.rich.<style_name>.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
#### links.label.richInheritPlainLabel
- **Type**: `boolean`
- **Default**: `true`
...
##### links.emphasis.lineStyle.dashOffset
- **Type**: `number`
- **Default**: `0`
...
##### links.emphasis.lineStyle.miterLimit
- **Type**: `number`
- **Default**: `10`
...
##### links.emphasis.label.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### links.emphasis.label.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### links.emphasis.label.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### links.emphasis.label.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### links.emphasis.label.rich.<style_name>.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### links.emphasis.label.rich.<style_name>.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### links.emphasis.label.rich.<style_name>.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### links.emphasis.label.rich.<style_name>.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### links.emphasis.label.richInheritPlainLabel
- **Type**: `boolean`
- **Default**: `true`
...
##### links.blur.lineStyle.width
- **Type**: `number`
- **Default**: `1`

线宽。

##### links.blur.lineStyle.type
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### links.blur.lineStyle.dashOffset
- **Type**: `number`
- **Default**: `0`
...
##### links.blur.lineStyle.miterLimit
- **Type**: `number`
- **Default**: `10`
...
##### links.blur.label.show
- **Type**: `boolean`
- **Default**: `false`

是否显示标签。

##### links.blur.label.position
- **Type**: `string|Array`
...
##### links.blur.label.offset
- **Type**: `Array`

是否对文字进行偏移。默认不偏移。例如：`[30, 40]` 表示文字在横向上偏移 `30`，纵向上偏移 `40`。

##### links.blur.label.color
- **Type**: `Color`
- **Default**: `'#fff'`
...
##### links.blur.label.fontSize
- **Type**: `number`
- **Default**: `12`

文字的字体大小。

##### links.blur.label.align
- **Type**: `string`
...
##### links.blur.label.verticalAlign
- **Type**: `string`
...
##### links.blur.label.backgroundColor
- **Type**: `string|Object`
- **Default**: `'transparent'`
...
##### links.blur.label.borderWidth
- **Type**: `number`
- **Default**: `0`

文字块边框宽度。

##### links.blur.label.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### links.blur.label.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### links.blur.label.borderRadius
- **Type**: `number|Array`
- **Default**: `0`

文字块的圆角。

##### links.blur.label.padding
- **Type**: `number|Array`
- **Default**: `0`
...
##### links.blur.label.shadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字块的背景阴影颜色。

##### links.blur.label.shadowBlur
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影长度。

##### links.blur.label.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 X 偏移。

##### links.blur.label.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 Y 偏移。

##### links.blur.label.width
- **Type**: `number`

文本显示宽度。

##### links.blur.label.height
- **Type**: `number`

文本显示高度。

##### links.blur.label.textBorderColor
- **Type**: `Color`
...
##### links.blur.label.textBorderWidth
- **Type**: `number`

文字本身的描边宽度。

##### links.blur.label.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### links.blur.label.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### links.blur.label.textShadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字本身的阴影颜色。

##### links.blur.label.textShadowBlur
- **Type**: `number`
- **Default**: `0`

文字本身的阴影长度。

##### links.blur.label.textShadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 X 偏移。

##### links.blur.label.textShadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 Y 偏移。

##### links.blur.label.overflow
- **Type**: `string`
- **Default**: `'none'`
...
##### links.blur.label.ellipsis
- **Type**: `string`
- **Default**: `'...'`

在`overflow`配置为`'truncate'`的时候，可以通过该属性配置末尾显示的文本。

##### links.blur.label.rich
- **Type**: `Object`
...
###### links.blur.label.rich.<style_name>.color
- **Type**: `Color`
- **Default**: `'#fff'`
...
###### links.blur.label.rich.<style_name>.fontStyle
- **Type**: `string`
- **Default**: `'normal'`
...
###### links.blur.label.rich.<style_name>.fontWeight
- **Type**: `string|number`
- **Default**: `'normal'`
...
###### links.blur.label.rich.<style_name>.fontFamily
- **Type**: `string`
- **Default**: `'sans-serif'`
...
###### links.blur.label.rich.<style_name>.fontSize
- **Type**: `number`
- **Default**: `12`

文字的字体大小。

###### links.blur.label.rich.<style_name>.align
- **Type**: `string`
...
###### links.blur.label.rich.<style_name>.verticalAlign
- **Type**: `string`
...
###### links.blur.label.rich.<style_name>.lineHeight
- **Type**: `number`
...
###### links.blur.label.rich.<style_name>.backgroundColor
- **Type**: `string|Object`
- **Default**: `'transparent'`
...
###### links.blur.label.rich.<style_name>.borderColor
- **Type**: `Color`
...
###### links.blur.label.rich.<style_name>.borderWidth
- **Type**: `number`
- **Default**: `0`

文字块边框宽度。

###### links.blur.label.rich.<style_name>.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### links.blur.label.rich.<style_name>.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### links.blur.label.rich.<style_name>.borderRadius
- **Type**: `number|Array`
- **Default**: `0`

文字块的圆角。

###### links.blur.label.rich.<style_name>.padding
- **Type**: `number|Array`
- **Default**: `0`
...
###### links.blur.label.rich.<style_name>.shadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字块的背景阴影颜色。

###### links.blur.label.rich.<style_name>.shadowBlur
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影长度。

###### links.blur.label.rich.<style_name>.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 X 偏移。

###### links.blur.label.rich.<style_name>.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 Y 偏移。

###### links.blur.label.rich.<style_name>.width
- **Type**: `number|string`
...
###### links.blur.label.rich.<style_name>.height
- **Type**: `number|string`
...
###### links.blur.label.rich.<style_name>.textBorderColor
- **Type**: `Color`
...
###### links.blur.label.rich.<style_name>.textBorderWidth
- **Type**: `number`

文字本身的描边宽度。

###### links.blur.label.rich.<style_name>.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### links.blur.label.rich.<style_name>.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### links.blur.label.rich.<style_name>.textShadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字本身的阴影颜色。

###### links.blur.label.rich.<style_name>.textShadowBlur
- **Type**: `number`
- **Default**: `0`

文字本身的阴影长度。

###### links.blur.label.rich.<style_name>.textShadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 X 偏移。

###### links.blur.label.rich.<style_name>.textShadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 Y 偏移。

##### links.blur.label.richInheritPlainLabel
- **Type**: `boolean`
- **Default**: `true`
...
##### links.select.lineStyle.width
- **Type**: `number`
- **Default**: `1`

线宽。

##### links.select.lineStyle.type
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### links.select.lineStyle.dashOffset
- **Type**: `number`
- **Default**: `0`
...
##### links.select.lineStyle.miterLimit
- **Type**: `number`
- **Default**: `10`
...
##### links.select.label.show
- **Type**: `boolean`
- **Default**: `false`

是否显示标签。

##### links.select.label.position
- **Type**: `string|Array`
...
##### links.select.label.offset
- **Type**: `Array`

是否对文字进行偏移。默认不偏移。例如：`[30, 40]` 表示文字在横向上偏移 `30`，纵向上偏移 `40`。

##### links.select.label.color
- **Type**: `Color`
- **Default**: `'#fff'`
...
##### links.select.label.fontSize
- **Type**: `number`
- **Default**: `12`

文字的字体大小。

##### links.select.label.align
- **Type**: `string`
...
##### links.select.label.verticalAlign
- **Type**: `string`
...
##### links.select.label.backgroundColor
- **Type**: `string|Object`
- **Default**: `'transparent'`
...
##### links.select.label.borderColor
- **Type**: `Color`
...
##### links.select.label.borderWidth
- **Type**: `number`
- **Default**: `0`

文字块边框宽度。

##### links.select.label.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### links.select.label.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### links.select.label.borderRadius
- **Type**: `number|Array`
- **Default**: `0`

文字块的圆角。

##### links.select.label.padding
- **Type**: `number|Array`
- **Default**: `0`
...
##### links.select.label.shadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字块的背景阴影颜色。

##### links.select.label.shadowBlur
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影长度。

##### links.select.label.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 X 偏移。

##### links.select.label.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 Y 偏移。

##### links.select.label.width
- **Type**: `number`

文本显示宽度。

##### links.select.label.height
- **Type**: `number`

文本显示高度。

##### links.select.label.textBorderColor
- **Type**: `Color`
...
##### links.select.label.textBorderWidth
- **Type**: `number`

文字本身的描边宽度。

##### links.select.label.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
##### links.select.label.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
##### links.select.label.textShadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字本身的阴影颜色。

##### links.select.label.textShadowBlur
- **Type**: `number`
- **Default**: `0`

文字本身的阴影长度。

##### links.select.label.textShadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 X 偏移。

##### links.select.label.textShadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 Y 偏移。

##### links.select.label.overflow
- **Type**: `string`
- **Default**: `'none'`
...
##### links.select.label.ellipsis
- **Type**: `string`
- **Default**: `'...'`

在`overflow`配置为`'truncate'`的时候，可以通过该属性配置末尾显示的文本。

##### links.select.label.rich
- **Type**: `Object`
...
###### links.select.label.rich.<style_name>.color
- **Type**: `Color`
- **Default**: `'#fff'`
...
###### links.select.label.rich.<style_name>.fontStyle
- **Type**: `string`
- **Default**: `'normal'`
...
###### links.select.label.rich.<style_name>.fontWeight
- **Type**: `string|number`
- **Default**: `'normal'`
...
###### links.select.label.rich.<style_name>.fontFamily
- **Type**: `string`
- **Default**: `'sans-serif'`
...
###### links.select.label.rich.<style_name>.fontSize
- **Type**: `number`
- **Default**: `12`

文字的字体大小。

###### links.select.label.rich.<style_name>.align
- **Type**: `string`
...
###### links.select.label.rich.<style_name>.verticalAlign
- **Type**: `string`
...
###### links.select.label.rich.<style_name>.lineHeight
- **Type**: `number`
...
###### links.select.label.rich.<style_name>.backgroundColor
- **Type**: `string|Object`
- **Default**: `'transparent'`
...
###### links.select.label.rich.<style_name>.borderColor
- **Type**: `Color`
...
###### links.select.label.rich.<style_name>.borderWidth
- **Type**: `number`
- **Default**: `0`

文字块边框宽度。

###### links.select.label.rich.<style_name>.borderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### links.select.label.rich.<style_name>.borderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### links.select.label.rich.<style_name>.borderRadius
- **Type**: `number|Array`
- **Default**: `0`

文字块的圆角。

###### links.select.label.rich.<style_name>.padding
- **Type**: `number|Array`
- **Default**: `0`
...
###### links.select.label.rich.<style_name>.shadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字块的背景阴影颜色。

###### links.select.label.rich.<style_name>.shadowBlur
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影长度。

###### links.select.label.rich.<style_name>.shadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 X 偏移。

###### links.select.label.rich.<style_name>.shadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字块的背景阴影 Y 偏移。

###### links.select.label.rich.<style_name>.width
- **Type**: `number|string`
...
###### links.select.label.rich.<style_name>.height
- **Type**: `number|string`
...
###### links.select.label.rich.<style_name>.textBorderColor
- **Type**: `Color`
...
###### links.select.label.rich.<style_name>.textBorderWidth
- **Type**: `number`

文字本身的描边宽度。

###### links.select.label.rich.<style_name>.textBorderType
- **Type**: `string|number|Array`
- **Default**: `'solid'`
...
###### links.select.label.rich.<style_name>.textBorderDashOffset
- **Type**: `number`
- **Default**: `0`
...
###### links.select.label.rich.<style_name>.textShadowColor
- **Type**: `Color`
- **Default**: `'transparent'`

文字本身的阴影颜色。

###### links.select.label.rich.<style_name>.textShadowBlur
- **Type**: `number`
- **Default**: `0`

文字本身的阴影长度。

###### links.select.label.rich.<style_name>.textShadowOffsetX
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 X 偏移。

###### links.select.label.rich.<style_name>.textShadowOffsetY
- **Type**: `number`
- **Default**: `0`

文字本身的阴影 Y 偏移。

##### links.select.label.richInheritPlainLabel
- **Type**: `boolean`
- **Default**: `true`
...
别名，同 [links](option.series-graph.md#links)
...
其中第一个参数 `value` 为 [data](option.series-graph.md#data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1.  通过 [x](option.series-graph.md#markPoint.data.x), [y](option.series-graph.md#markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-graph.md#markPoint.data.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](option.series-graph.md#markPoint.data.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](option.series-graph.md#markPoint.data.valueIndex) 或者 [valueDim](option.series-graph.md#markPoint.data.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
标注名称。定义后可在 [label formatter](option.series-graph.md#markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
在使用 [type](option.series-graph.md#markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

#### markPoint.data.valueDim
- **Type**: `string`

在使用 [type](option.series-graph.md#markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](option.series-graph.md#markPoint.data.x) 和 [y](option.series-graph.md#markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](option.series-graph.md#markLine.data.0.symbol)。
...
1.  通过 [x](option.series-graph.md#markLine.data.0.x), [y](option.series-graph.md#markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-graph.md#markLine.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](option.series-graph.md#markLine.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](option.series-graph.md#markLine.data.0.valueIndex) 或者 [valueDim](option.series-graph.md#markLine.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](option.series-graph.md#markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markLine.data.0.valueDim
- **Type**: `string`

在使用 [type](option.series-graph.md#markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称。定义后可在 [label formatter](option.series-graph.md#markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
1.  通过 [x](option.series-graph.md#markArea.data.0.x), [y](option.series-graph.md#markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-graph.md#markArea.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](option.series-graph.md#markArea.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](option.series-graph.md#markArea.data.0.valueIndex) 或者 [valueDim](option.series-graph.md#markPoint.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](option.series-graph.md#markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markArea.data.0.valueDim
- **Type**: `string`

在使用 [type](option.series-graph.md#markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](option.series-graph.md#markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
关系图系列（graph series）离容器左侧的距离。

`left` 的值可以是像 `20` 这样的具体像素值，可以是像 `'20%'` 这样相对于容器宽度的百分比，也可以是 `'left'`, `'center'`, `'right'`。

如果 `left` 的值为 `'left'`, `'center'`, `'right'`，组件会根据相应的位置自动对齐。

> 注：如果发现图形被拉伸了不符合预期，参见 [preserveAspect](option.series-graph.md#preserveAspect)。

## top
- **Type**: `string|number`
- **Default**: `'middle'`

关系图系列（graph series）离容器上侧的距离。
...
为 graph 系列 分配的 `矩形区域` 是通过以下配置决定的： [series-graph.left](option.series-graph.md#left) / [.right](option.series-graph.md#right) / [.top](option.series-graph.md#top) / [.bottom](option.series-graph.md#bottom) / [.width](option.series-graph.md#width) / [.height](option.series-graph.md#height)。

但是这个矩形的 `aspect ratio` 不一定和内容的原始 `aspect ratio` 一致，从而可能导致内容被拉伸而失真。

`preserveAspect` 的不同选项有如下效果：

*   `null` / `undefined` / `false`（默认）：不会保持内容的原始 `aspect ratio`，而是拉伸内容以填满 `graph 系列矩形区域`。拉伸有可能导致图形失真。
*   `'contain'` / `true`：保持内容的 `aspect ratio`。内容的包围盒被完整地包含在 `graph 系列矩形区域` 中，并尽可能放大以触达边界。此时可使用 [preserveAspectAlign](option.series-graph.md#preserveAspectAlign) 和 [preserveAspectVerticalAlign](option.series-graph.md#preserveAspectVerticalAlign) 调整位置。
*   `'cover'`：保持内容的原始 `aspect ratio`。内容的包围盒会覆盖整个 `graph 系列矩形区域`，并尽可能缩小以触达边界。此时可使用 [preserveAspectAlign](option.series-graph.md#preserveAspectAlign) 和 [preserveAspectVerticalAlign](option.series-graph.md#preserveAspectVerticalAlign) 调整位置。
...
参见 [preserveAspect](option.series-graph.md#preserveAspect)。
