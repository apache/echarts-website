...
指定另一个坐标系组件，本 `series-pie` 布局在那个坐标系中。
...
*   `'geo'`
    ...
    布局在一个 [地理坐标系](option.geo.md) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](option.series-pie.md#geoIndex) 或 [geoId](option.series-pie.md#geoId) 指定所使用的地理坐标系。
    
    参见示例 [地理坐标系中的饼图](https://echarts.apache.org/examples/zh/editor.html?c=map-iceland-pie&edit=1&reset=1)。
    

*   `'calendar'`
    
    布局在一个 [日历坐标系](option.calendar.md) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](option.series-pie.md#calendarIndex) 或 [calendarId](option.series-pie.md#calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](option.matrix.md)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](option.series-pie.md#matrixIndex) 或 [matrixId](option.series-pie.md#matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-pie.coordinateSystemUsage](option.series-pie.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-pie.md#coordinateSystem) 上布局本 `series-pie`。
...
*   `'box'`：**（不适用于 [series-pie](option.series-pie.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-pie.coords](option.series-pie.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-pie.coordinateSystem](option.series-pie.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-pie.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
在此场景下，[series-pie.center](option.series-pie.md#center) 和 [series-pie.coord](option.series-pie.md#coord) 起同样作用。

[例子：地理坐标系中的饼图](https://echarts.apache.org/examples/zh/editor.html?c=map-iceland-pie&edit=1&reset=1)

> 注：当 [coordinateSystemUsage](option.series-pie.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
当值为 `'auto'` 时，根据 [startAngle](option.series-pie.md#startAngle) 自动计算结束角度，以确保是一个完整的圆。
...
如果不需要开启该策略，例如[圆环图](https://echarts.apache.org/examples/zh/editor.html?c=pie-doughnut)这个例子中需要强制所有标签放在中心位置，可以将该值设为 `false`。
...
*   `'outside'`
    ...
    饼图扇区外侧，通过[视觉引导线](option.series-pie.md#labelLine)连到相应的扇区。
...
*   `'center'`
    ...
    在饼图中心位置。见[圆环图示例](https://echarts.apache.org/examples/zh/editor.html?c=pie-doughnut)
...
*   `'none'`（默认值）：label line 的长度为固定值，分别为 [labelLine.length](option.series-pie.md#labelLine.length) 及 [labelLine.length2](option.series-pie.md#labelLine.length2)。
*   `'labelLine'`：label line 的末端对齐，其中最短的长度由 [labelLine.length2](option.series-pie.md#labelLine.length2) 决定。
*   `'edge'`：文字对齐，文字的边距由 [label.edgeDistance](option.series-pie.md#label.edgeDistance) 决定。

### label.edgeDistance
- **Type**: `string|number`
- **Default**: `'25%'`

文字边距，仅当 [label.position](option.series-pie.md#label.position) 为 `'outer'` 并且 [label.alignTo](option.series-pie.md#label.alignTo) 为 `'edge'` 时有效。

通常来说，对于移动端等分辨率较小的情况，`edgeDistance` 值设为比较小的值（比如 `10`）能在有限的空间内显示更多文字，而不是被裁剪为 `...`。但是对于分辨率更大的场景，百分比的值可以避免 label line 过长。如果你需要在不同分辨率下使用，建议使用[响应式图表设计](../tutorial.md#移动端自适应)为不同的分辨率设置不同的 `edgeDistance` 值。

### label.bleedMargin
- **Type**: `number`
- **Default**: `10`

文字的出血线大小，超过出血线的文字将被裁剪为 `'...'`。仅当 [label.position](option.series-pie.md#label.position) 为 `'outer'` 并且 [label.alignTo](option.series-pie.md#label.alignTo) 为 `'none'` 或 `'labelLine'` 的情况有效。
...
标签的视觉引导线配置。在 [label 位置](option.series-pie.md#label.position) 设置为`'outside'`的时候会显示视觉引导线。
...
高亮后扇区的放大尺寸，在开启 [emphasis.scale](option.series-pie.md#emphasis.scale) 后有效。
...
淡出状态的扇区和标签样式。开启 [emphasis.focus](option.series-pie.md#emphasis.focus) 后有效。
...
选中状态的扇区和标签样式。开启 [selectedMode](option.series-pie.md#selectedMode) 后有效。
...
其中第一个参数 `value` 为 [data](option.series-pie.md#data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1.  通过 [x](option.series-pie.md#markPoint.data.x), [y](option.series-pie.md#markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称。定义后可在 [label formatter](option.series-pie.md#markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](option.series-pie.md#markPoint.data.x) 和 [y](option.series-pie.md#markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](option.series-pie.md#markLine.data.0.symbol)。
...
1.  通过 [x](option.series-pie.md#markLine.data.0.x), [y](option.series-pie.md#markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称。定义后可在 [label formatter](option.series-pie.md#markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
1.  通过 [x](option.series-pie.md#markArea.data.0.x), [y](option.series-pie.md#markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](option.series-pie.md#markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
