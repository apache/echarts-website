...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](option.calendar.md) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](option.polar.md#calendarIndex) 或 [calendarId](option.polar.md#calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](option.matrix.md)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](option.polar.md#matrixIndex) 或 [matrixId](option.polar.md#matrixId) 指定所使用的矩阵坐标系。
...
也参见 [polar.coordinateSystemUsage](option.polar.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.polar.md#coordinateSystem) 上布局本 `polar`。

在大多数情况下，无需显式指定 `coordinateSystemUsage`，除非默认行为不符合预期。

可选值：

*   `'data'`：**（不适用于 [polar](option.polar.md)）**
...
*   `'box'`：
    ...
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [polar.coords](option.polar.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[polar.coordinateSystem](option.polar.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.polar.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.polar.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
