...
指定另一个坐标系组件，本 `series-funnel` 布局在那个坐标系中。
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](option.calendar.md) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](option.series-funnel.md#calendarIndex) 或 [calendarId](option.series-funnel.md#calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](option.matrix.md)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](option.series-funnel.md#matrixIndex) 或 [matrixId](option.series-funnel.md#matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-funnel.coordinateSystemUsage](option.series-funnel.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-funnel.md#coordinateSystem) 上布局本 `series-funnel`。
...
*   `'box'`：**（不适用于 [series-funnel](option.series-funnel.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-funnel.coords](option.series-funnel.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-funnel.coordinateSystem](option.series-funnel.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-funnel.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.series-funnel.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
数据最小值 [min](option.series-funnel.md#min) 映射的宽度。

可以是绝对的像素大小，也可以是相对[布局宽度](option.series-funnel.md#width)的百分比，如果需要最小值的图形并不是尖端三角，可通过设置该属性实现。

## maxSize
- **Type**: `number|string`
- **Default**: `'100%'`

数据最大值 [max](option.series-funnel.md#max) 映射的宽度。

可以是绝对的像素大小，也可以是相对[布局宽度](option.series-funnel.md#width)的百分比。
...
*   `'left'` 漏斗图左侧，[orient](option.series-funnel.md#orient) 为`'vertical'`时有效。
*   `'right'` 漏斗图右侧，[orient](option.series-funnel.md#orient) 为`'vertical'`时有效。
*   `'top'` 漏斗图上侧，[orient](option.series-funnel.md#orient) 为`'horizontal'`时有效。
*   `'bottom'` 漏斗图下侧，[orient](option.series-funnel.md#orient) 为`'horizontal'`时有效。
*   `'inside'` 漏斗图梯形内部。
...
在不是配置为内部的时候标签可以通过[视觉引导线](option.series-funnel.md#labelLine)连到相应的梯形。 通过[视觉引导线](option.series-funnel.md#labelLine)连到相应的梯形。
...
标签的视觉引导线样式，在 [label 位置](option.series-funnel.md#label.position) 设置为`'left'`或者`'right'`的时候会显示视觉引导线。
...
淡出时的图形样式和标签样式。开启 [emphasis.focus](option.series-funnel.md#emphasis.focus) 后有效
...
数据选中时的图形样式和标签样式。开启 [selectedMode](option.series-funnel.md#selectedMode) 后有效。
...
其中第一个参数 `value` 为 [data](option.series-funnel.md#data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1.  通过 [x](option.series-funnel.md#markPoint.data.x), [y](option.series-funnel.md#markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称。定义后可在 [label formatter](option.series-funnel.md#markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](option.series-funnel.md#markPoint.data.x) 和 [y](option.series-funnel.md#markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](option.series-funnel.md#markLine.data.0.symbol)。
...
1.  通过 [x](option.series-funnel.md#markLine.data.0.x), [y](option.series-funnel.md#markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称。定义后可在 [label formatter](option.series-funnel.md#markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
1.  通过 [x](option.series-funnel.md#markArea.data.0.x), [y](option.series-funnel.md#markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](option.series-funnel.md#markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
