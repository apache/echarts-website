...
也参见示例 [USA Population Estimates](https://echarts.apache.org/examples/zh/editor.html?c=map-usa)。
...
也参见示例 [Beef Cuts](https://echarts.apache.org/examples/zh/editor.html?c=geo-beef-cuts)。
...
`center` 一般用于控制或获取 [平移缩放（roam）](option.series-map.md#roam) 时地图的位置。当平移缩放时，`center` 和 [zoom](option.series-map.md#zoom) 的值会被相应改变。

注意：`center` 的值基于的是对象布局的原始坐标系而非画布的坐标。 如果想使用画布坐标调整绘制对象的位置和大小，使用：[series-map.left](option.series-map.md#left) / [.right](option.series-map.md#right) / [.top](option.series-map.md#top) / [.bottom](option.series-map.md#bottom) / [.width](option.series-map.md#width) / [.height](option.series-map.md#height) 或者 [series-map.layoutCenter](option.series-map.md#layoutCenter) / [layoutSize](option.series-map.md#layoutSize)。

`center` 默认使用原始坐标（经纬度）。如果设置了 [projection](option.series-map.md#projection) 则用投影后的坐标。
...
百分比（字符串）值也可被用在 `center` 中，例如 `'30%'`，基于包围盒（由经纬度的最大最小值决定；或者，在设置了 [projection](option.series-map.md#projection) 时，由投影后得到的最大最小坐标决定）。你可以使用 `'0%'` 将包围盒的顶部或左侧对齐到视口（画布）中心，或使用 `'100%'` 将右侧或底部对齐到视口中心，或使用 `'50%'` 表示把整个图放于视口中心。
...
当 [平移缩放（roam）](option.series-map.md#roam) 时，[center](option.series-map.md#center) 和 `zoom` 的值会被相应改变。
...
[缩放](option.series-map.md#roam) 的极限控制，通过 `min` 和 `max` 限制最小和最大的缩放值。
...
当缩放和平移时，[center](option.series-map.md#center) 和 [zoom](option.series-map.md#zoom) 的值会被相应改变。
...
[缩放和平移（roam）](option.series-map.md#roam) 可被鼠标触发。
...
这个参数用于 scale 地图的长宽比。如果设置了 [projection](option.series-map.md#projection) 则无效。

地图最终计算得到的 `pixelWidth` 和 `pixelHeight` 将满足以下关系：`pixelWidth / pixelHeight = lngSpan / latSpan * aspectScale`（假设未指定 [projection](option.series-map.md#projection)，且 [preserveAspect](option.series-map.md#preserveAspect) 设为保持长宽比）。

当不使用真正的投影公式（[projection](option.series-map.md#projection)）时，GeoJSON 里的经纬度会被线性映射到像素坐标。`aspectScale` 提供了一种简单方式，用于视觉上补偿这种映射所造成的形变（由于地球是球形，经度对应的物理尺寸在高纬度地区会收缩）。例如，`aspectScale` 可以通过以下公式粗略计算：`aspectScale = Math.cos(center_latitude * Math.PI / 180)`，这与正弦投影（sinusoidal projection）相似。
...
> 注：如果发现图形被拉伸了不符合预期，参见 [preserveAspect](option.series-map.md#preserveAspect)。

> 注：series-map 有两种盒布局方式。你可以使用任一种：
> 
...
`layoutCenter` 和 `layoutSize` 用于定义 地图系列 所拥有的矩形区域，其中`layoutCenter` 定义了区域中心在容器中的位置，`layoutSize` 定义了区域的大小。如下示例：
...
地图的大小，见 [layoutCenter](series-map.layoutCenter)。支持相对于容器宽高的百分比或者绝对的像素大小。
...
为 地图系列 分配的 `矩形区域` 是通过以下配置决定的： [series-map.left](option.series-map.md#left) / [.right](option.series-map.md#right) / [.top](option.series-map.md#top) / [.bottom](option.series-map.md#bottom) / [.width](option.series-map.md#width) / [.height](option.series-map.md#height) / [.aspectScale](option.series-map.md#aspectScale)。

但是这个矩形的 `aspect ratio` 不一定和内容的原始 `aspect ratio` 一致，从而可能导致内容被拉伸而失真。

`preserveAspect` 的不同选项有如下效果：

*   `null` / `undefined` / `false`（默认）：不会保持内容的原始 `aspect ratio`，而是拉伸内容以填满 `地图系列矩形区域`。拉伸有可能导致图形失真。
*   `'contain'` / `true`：保持内容的 `aspect ratio`。内容的包围盒被完整地包含在 `地图系列矩形区域` 中，并尽可能放大以触达边界。此时可使用 [preserveAspectAlign](option.series-map.md#preserveAspectAlign) 和 [preserveAspectVerticalAlign](option.series-map.md#preserveAspectVerticalAlign) 调整位置。
*   `'cover'`：保持内容的原始 `aspect ratio`。内容的包围盒会覆盖整个 `地图系列矩形区域`，并尽可能缩小以触达边界。此时可使用 [preserveAspectAlign](option.series-map.md#preserveAspectAlign) 和 [preserveAspectVerticalAlign](option.series-map.md#preserveAspectVerticalAlign) 调整位置。

注意：当使用 [layoutCenter](option.series-map.md#layoutCenter) 和 [layoutSize](option.series-map.md#layoutSize) 时，始终会保留宽高比，无论 `preserveAspect` 配置为何值。
...
参见 [preserveAspect](option.series-map.md#preserveAspect)。
...
指定另一个坐标系组件，本 `series-map` 布局在那个坐标系中。

可选值：

*   `'geo'`
    
    布局在一个 [地理坐标系](option.geo.md) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](option.series-map.md#geoIndex) 或 [geoId](option.series-map.md#geoId) 指定所使用的地理坐标系。
...
也参见 [series-map.coordinateSystemUsage](option.series-map.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-map.md#coordinateSystem) 上布局本 `series-map`。
...
*   `'box'`：**（不适用于 [series-map](option.series-map.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-map.coords](option.series-map.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-map.coordinateSystem](option.series-map.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-map.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.series-map.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
默认情况下，map series 会自己生成内部专用的 `geo` 组件。但是也可以用这个 `geoIndex` 指定一个 [geo](option.geo.md) 组件。这样的话，map 和 其他 series（例如散点图）就可以共享一个 [geo](option.geo.md) 组件了。并且，[geo](option.geo.md) 组件的颜色也可以被这个 map series 控制，从而用 [visualMap](../option.md#visualMap) 来更改。

当设定了 `geoIndex` 后，[series-map.map](option.series-map.md#map) 属性，以及 [series-map.itemStyle](option.series-map.md#itemStyle) 等样式配置不再起作用，而是采用 [geo](option.geo.md) 中的相应属性。
...
当没有指定 `geoIndex` 和 `geoId` 时，map 系列会为自身创建一个独享的 `geo` 组件。但也可以通过 `geoIndex` 或 `geoId` 指定一个外部的 [geo 组件](option.geo.md)，从而与其他系列（如 [pie](option.series-pie.md)）共享该坐标系。此外，外部 [geo 组件](option.geo.md) 的区域颜色也可以由 map 系列通过 [visualMap](../option.md#visualMap) 控制。

当指定了 `geoIndex` 或 `geoId` 时，[series-map.map](option.series-map.md#map) 以及诸如 [series-map.itemStyle](option.series-map.md#itemStyle) 等样式配置将不再生效，而会采用 [geo 组件](option.geo.md) 中对应的配置。
...
多个拥有相同[地图类型](option.series-map.md#map)的系列会使用同一个地图展现，如果多个系列都在同一个区域有值，ECharts 会对这些值统计得到一个数据。这个配置项就是用于配置统计的方式，目前有：
...
在图例相应区域显示图例的颜色标识（系列标识的小圆点），存在 [legend](option.legend.md) 组件时生效。
...
其中第一个参数 `value` 为 [data](option.series-map.md#data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1.  通过 [x](option.series-map.md#markPoint.data.x), [y](option.series-map.md#markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-map.md#markPoint.data.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
...
标注名称。定义后可在 [label formatter](option.series-map.md#markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。

#### markPoint.data.valueIndex
- **Type**: `number`

在使用 [type](option.series-map.md#markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

#### markPoint.data.valueDim
- **Type**: `string`

在使用 [type](option.series-map.md#markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](option.series-map.md#markPoint.data.x) 和 [y](option.series-map.md#markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](option.series-map.md#markLine.data.0.symbol)。
...
1.  通过 [x](option.series-map.md#markLine.data.0.x), [y](option.series-map.md#markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-map.md#markLine.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
...
在使用 [type](option.series-map.md#markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markLine.data.0.valueDim
- **Type**: `string`

在使用 [type](option.series-map.md#markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称。定义后可在 [label formatter](option.series-map.md#markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
1.  通过 [x](option.series-map.md#markArea.data.0.x), [y](option.series-map.md#markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-map.md#markArea.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
...
在使用 [type](option.series-map.md#markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markArea.data.0.valueDim
- **Type**: `string`

在使用 [type](option.series-map.md#markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](option.series-map.md#markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
