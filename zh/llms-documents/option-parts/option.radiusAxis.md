...
*   `'category'` 类目轴，适用于离散的类目数据。为该类型时类目数据可自动从 [series.data](../option.md#series.data) 或 [dataset.source](option.dataset.md#source) 中取，或者可通过 [radiusAxis.data](option.radiusAxis.md#data) 设置类目数据。
...
坐标轴名称的颜色，默认取 [axisLine.lineStyle.color](option.radiusAxis.md#axisLine.lineStyle.color)。
...
类目轴中 `boundaryGap` 可以配置为 `true` 和 `false`。默认为 `true`，这时候[刻度](option.radiusAxis.md#axisTick)只是作为分隔线，标签和数据点都会在两个[刻度](option.radiusAxis.md#axisTick)之间的带(band)中间。

非类目轴，包括时间，数值，对数轴，`boundaryGap`是一个两个值的数组，分别表示数据最小值和最大值的延伸范围，可以直接设置数值或者相对的百分比，在设置 [min](option.radiusAxis.md#min) 和 [max](option.radiusAxis.md#max) 后无效。 **示例：**
...
也参见 [boundaryGap](option.radiusAxis.md#boundaryGap)。这两个配置项的功能因为历史原因有所重叠。只有在 `boundaryGap: false, containShape: false` 时，类目坐标轴（`axis.type: 'category'`）中的图形才可能超出坐标轴范围。
...
只在数值轴、对数轴、时间轴中（[type](option.radiusAxis.md#type): 'value'、'log' 或 'time'）有效。
...
**与 [min](option.radiusAxis.md#min) 的区别：**
...
**与 [max](option.radiusAxis.md#max) 的区别：**
...
只在数值轴中（[type](option.radiusAxis.md#type): 'value'）有效。

是否是脱离 0 值比例。设置成 `true` 后坐标刻度不会强制包含零刻度。在双数值轴的散点图中比较有用。

在设置 [min](option.radiusAxis.md#min) 和 [max](option.radiusAxis.md#max) 之后该配置项无效。
...
只在数值轴或时间轴中（[type](option.radiusAxis.md#type): 'value' 或 'time'）有效。
...
例如，在时间轴（（[type](option.radiusAxis.md#type): 'time'））可以设置成 `3600 * 24 * 1000` 保证坐标轴分割刻度最大为一天。
...
因为 [splitNumber](option.radiusAxis.md#splitNumber) 是预估的值，实际根据策略计算出来的刻度可能无法达到想要的效果，这时候可以使用 interval 配合 [min](option.radiusAxis.md#min)、[max](option.radiusAxis.md#max) 强制设定刻度划分，一般不建议使用。

无法在类目轴中使用。在时间轴（[type](option.radiusAxis.md#type): 'time'）中需要传时间戳，在对数轴（[type](option.radiusAxis.md#type): 'log'）中需要传指数值。

## logBase
- **Type**: `number`
- **Default**: `10`

对数轴的底数，只在对数轴中（[type](option.radiusAxis.md#type): 'log'）有效。
...
坐标轴刻度的显示间隔，在类目轴中有效。默认同 [axisLabel.interval](option.radiusAxis.md#axisLabel.interval) 一样。
...
刻度线的颜色，默认取 [axisLine.lineStyle.color](option.radiusAxis.md#axisLine.lineStyle.color)。
...
注意：次刻度线无法在类目轴（[type](option.radiusAxis.md#type): `'category'`）中使用。
...
**如果使用了 [axis break](radiusAxis.breaks)**
...
**对于时间轴（[`radiusAxis.type: 'time'`](option.radiusAxis.md#type)）**
...
刻度标签文字的颜色，默认取 [axisLine.lineStyle.color](option.radiusAxis.md#axisLine.lineStyle.color)。支持回调函数，格式如下
...
坐标轴分隔线的显示间隔，在类目轴中有效。默认同 [axisLabel.interval](option.radiusAxis.md#axisLabel.interval) 一样。
...
坐标轴在 [grid](option.grid.md) 区域中的次分隔线。次分割线会对齐次刻度线 [minorTick](option.radiusAxis.md#minorTick)
...
坐标轴分隔区域的显示间隔，在类目轴中有效。默认同 [axisLabel.interval](option.radiusAxis.md#axisLabel.interval) 一样。
...
类目数据，在类目轴（[type](option.radiusAxis.md#type): `'category'`）中有效。

如果没有设置 [type](option.radiusAxis.md#type)，但是设置了 `axis.data`，则认为 `type` 是 `'category'`。

如果设置了 [type](option.radiusAxis.md#type) 是 `'category'`，但没有设置 `axis.data`，则 `axis.data` 的内容会自动从 [series.data](../option.md#series.data) 中获取，这会比较方便。不过注意，`axis.data` 指明的是 `'category'` 轴的取值范围。如果不指定而是从 [series.data](../option.md#series.data) 中获取，那么只能获取到 [series.data](../option.md#series.data) 中出现的值。比如说，假如 [series.data](../option.md#series.data) 为空时，就什么也获取不到。
...
坐标轴 tooltip 设置，注意需设置 [triggerEvent](option.radiusAxis.md#triggerEvent) 为 `true` 并启用全局 [option.tooltip](option.tooltip.md) 组件。
...
半径轴组件的所有图形的`z`值。控制图形的前后顺序。`z`值小的图形会被`z`值大的图形覆盖。
