...
指定另一个坐标系组件，本 `series-lines` 布局在那个坐标系中。

可选值：

*   `'cartesian2d'`
    
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](option.grid.md) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](option.series-lines.md#xAxisIndex) 和 [yAxisIndex](option.series-lines.md#yAxisIndex) 或者 [xAxisId](option.series-lines.md#xAxisId) 和 [yAxisId](option.series-lines.md#yAxisId) 指定所使用的坐标轴。
...
*   `'geo'`
    ...
    布局在一个 [地理坐标系](option.geo.md) 中。当一个 ECharts 实例中存在多个地理坐标系时，须通过 [geoIndex](option.series-lines.md#geoIndex) 或 [geoId](option.series-lines.md#geoId) 指定所使用的地理坐标系。
    

*   `'singleAxis'`
    
    布局在一个 [单轴坐标系](option.singleAxis.md) 中。当一个 ECharts 实例中存在多个单轴坐标系时，须通过 [singleAxisIndex](option.series-lines.md#singleAxisIndex) 或 [singleAxisId](option.series-lines.md#singleAxisId) 指定所使用的单轴标系。
...
也参见 [series-lines.coordinateSystemUsage](option.series-lines.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-lines.md#coordinateSystem) 上布局本 `series-lines`。
...
*   `'box'`：**（不适用于 [series-lines](option.series-lines.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-lines.coords](option.series-lines.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-lines.coordinateSystem](option.series-lines.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-lines.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.series-lines.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
默认为 `false`，只能用于绘制只有两个端点的线段，线段可以通过 [lineStyle.curveness](option.series-lines.md#lineStyle.curveness) 配置为曲线。

如果该配置项为 `true`，则可以在 [data.coords](option.series-lines.md#data.coords) 中设置多于 2 个的顶点用来绘制多段线，在绘制路线轨迹的时候比较有用，见示例 [北京公交路线](https://echarts.apache.org/examples/zh/editor.html?c=lines-bmap-bus)，设置为多段线后 [lineStyle.curveness](option.series-lines.md#lineStyle.curveness) 无效。

## effect
- **Type**: `Object`

线特效的配置，见示例 [模拟迁徙](https://echarts.apache.org/examples/zh/editor.html?c=geo-lines) 和 [北京公交路线](https://echarts.apache.org/examples/zh/editor.html?c=lines-bmap-effect)

**注意：** 所有带有尾迹特效的图表需要单独放在一个层，也就是需要单独设置 [zlevel](option.series-lines.md#zlevel)，同时建议关闭该层的动画（[animation](option.series-lines.md#animation): false）。不然位于同个层的其它系列的图形，和动画的[标签](option.series-lines.md#label)也会产生不必要的残影。
...
配置特效图形的移动动画是否是固定速度，单位`像素/秒`，设置为大于 0 的值后会忽略 [period](option.series-lines.md#effect.period) 配置项。
...
特效标记的颜色，默认取 [lineStyle.color](option.series-lines.md#lineStyle.color)。
...
开启后配合 [largeThreshold](option.series-lines.md#largeThreshold) 在数据量大于指定阈值的时候对绘制进行优化。

缺点：优化后不能自定义设置单个数据项的样式，不能启用 [effect](option.series-lines.md#effect)。
...
线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定。 具体支持的格式可以参考 [标线的 symbol](option.series-line.md#markLine.data.0.symbol)
...
线的颜色。 默认从[option.color 调色盘](../option.md#color) 获取颜色。
...
标签相关配置。在 [polyline](option.series-lines.md#polyline) 设置为 `true` 时无效。
...
淡出的线条和标签样式。开启 [emphasis.focus](option.series-lines.md#emphasis.focus) 后有效。
...
选中的线条和标签样式。开启 [selectedMode](option.series-lines.md#selectedMode) 后有效。
...
一个包含两个到多个二维坐标的数组。在 [polyline](option.series-lines.md#polyline) 设置为 `true` 时支持多于两个的坐标。
...
单个数据（单条线）的标签设置。在 [polyline](option.series-lines.md#polyline) 设置为 `true` 时无效。
...
其中第一个参数 `value` 为 [data](option.series-lines.md#data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1.  通过 [x](option.series-lines.md#markPoint.data.x), [y](option.series-lines.md#markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称。定义后可在 [label formatter](option.series-lines.md#markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](option.series-lines.md#markPoint.data.x) 和 [y](option.series-lines.md#markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](option.series-lines.md#markLine.data.0.symbol)。
...
1.  通过 [x](option.series-lines.md#markLine.data.0.x), [y](option.series-lines.md#markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称。定义后可在 [label formatter](option.series-lines.md#markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
1.  通过 [x](option.series-lines.md#markArea.data.0.x), [y](option.series-lines.md#markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](option.series-lines.md#markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
路径图组件的所有图形的`z`值。控制图形的前后顺序。`z`值小的图形会被`z`值大的图形覆盖。
