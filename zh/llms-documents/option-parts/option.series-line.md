...
指定另一个坐标系组件，本 `series-line` 布局在那个坐标系中。

可选值：

*   `'cartesian2d'`
    
    布局在一个二维 [直角坐标系（也称笛卡尔坐标系）](option.grid.md) 中。当一个 ECharts 实例中存在多个 x 坐标轴（`xAxis`）时或者多个 y 坐标轴（`yAxis`）时，须通过 [xAxisIndex](option.series-line.md#xAxisIndex) 和 [yAxisIndex](option.series-line.md#yAxisIndex) 或者 [xAxisId](option.series-line.md#xAxisId) 和 [yAxisId](option.series-line.md#yAxisId) 指定所使用的坐标轴。
...
*   `'polar'`
    ...
    布局在一个 [极坐标系](option.polar.md) 中。当一个 ECharts 实例中存在多个极坐标系时，须通过 [polarIndex](option.series-line.md#polarIndex) 或 [polarId](option.series-line.md#polarId) 指定所使用的极坐标系。
    

*   `'singleAxis'`
    
    布局在一个 [单轴坐标系](option.singleAxis.md) 中。当一个 ECharts 实例中存在多个单轴坐标系时，须通过 [singleAxisIndex](option.series-line.md#singleAxisIndex) 或 [singleAxisId](option.series-line.md#singleAxisId) 指定所使用的单轴标系。
...
也参见 [series-line.coordinateSystemUsage](option.series-line.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-line.md#coordinateSystem) 上布局本 `series-line`。
...
*   `'box'`：**（不适用于 [series-line](option.series-line.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-line.coords](option.series-line.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-line.coordinateSystem](option.series-line.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-line.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.series-line.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
其中第一个参数 `value` 为 [data](option.series-line.md#data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
堆叠数值的计算方式，仅在设置了 [stack](option.series-line.md#stack) 属性后生效。可选值：
...
**注：** 修改 `lineStyle` 中的颜色不会影响图例颜色，如果需要图例颜色和折线图颜色一致，需修改 [itemStyle.color](option.series-line.md#itemStyle.color)，线条颜色默认也会取该颜色。
...
*   `'start'` 填充坐标轴底部（非 [`inverse`](option.yAxis.md#inverse) 情况是最小值）到数据间的区域
*   `'end'` 填充坐标轴顶部（非 [`inverse`](option.yAxis.md#inverse) 情况是最大值）到数据间的区域
...
折线图的淡出状态。开启 [emphasis.focus](option.series-line.md#emphasis.focus) 后有效。
...
折线图的选中状态。开启 [selectedMode](option.series-line.md#selectedMode) 后有效。
...
如果需要修改平滑算法，请参考 [smoothMonotone](option.series-line.md#smoothMonotone)。
...
![](../../documents/asset/img/smooth-monotone-x.png)
...
1.  通过 [x](option.series-line.md#markPoint.data.x), [y](option.series-line.md#markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-line.md#markPoint.data.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](option.series-line.md#markPoint.data.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](option.series-line.md#markPoint.data.valueIndex) 或者 [valueDim](option.series-line.md#markPoint.data.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
标注名称。定义后可在 [label formatter](option.series-line.md#markPoint.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
在使用 [type](option.series-line.md#markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

#### markPoint.data.valueDim
- **Type**: `string`

在使用 [type](option.series-line.md#markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
可选值：`'container'` 或 `'coordinate'`。指定 [x](option.series-line.md#markPoint.data.x) 和 [y](option.series-line.md#markPoint.data.y) 是相对于整个图表容器（`'container'`，默认值）还是直角坐标系（`'coordinate'`）而言的位置。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](option.series-line.md#markLine.data.0.symbol)。
...
1.  通过 [x](option.series-line.md#markLine.data.0.x), [y](option.series-line.md#markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-line.md#markLine.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](option.series-line.md#markLine.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](option.series-line.md#markLine.data.0.valueIndex) 或者 [valueDim](option.series-line.md#markLine.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](option.series-line.md#markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markLine.data.0.valueDim
- **Type**: `string`

在使用 [type](option.series-line.md#markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称。定义后可在 [label formatter](option.series-line.md#markLine.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
1.  通过 [x](option.series-line.md#markArea.data.0.x), [y](option.series-line.md#markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。

2.  用 [coord](option.series-line.md#markArea.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。
    
3.  直接用 [type](option.series-line.md#markArea.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](option.series-line.md#markArea.data.0.valueIndex) 或者 [valueDim](option.series-line.md#markPoint.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](option.series-line.md#markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### markArea.data.0.valueDim
- **Type**: `string`

在使用 [type](option.series-line.md#markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标注名称，将会作为文字显示。定义后可在 [label formatter](option.series-line.md#markArea.data.label.formatter) 中作为**数据名** `{b}` 模板变量使用。
...
折线图组件的所有图形的`z`值。控制图形的前后顺序。`z`值小的图形会被`z`值大的图形覆盖。
