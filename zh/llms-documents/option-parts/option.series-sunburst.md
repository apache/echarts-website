...
指定另一个坐标系组件，本 `series-sunburst` 布局在那个坐标系中。
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](option.calendar.md) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](option.series-sunburst.md#calendarIndex) 或 [calendarId](option.series-sunburst.md#calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](option.matrix.md)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](option.series-sunburst.md#matrixIndex) 或 [matrixId](option.series-sunburst.md#matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-sunburst.coordinateSystemUsage](option.series-sunburst.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-sunburst.md#coordinateSystem) 上布局本 `series-sunburst`。
...
*   `'box'`：**（不适用于 [series-sunburst](option.series-sunburst.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-sunburst.coords](option.series-sunburst.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-sunburst.coordinateSystem](option.series-sunburst.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-sunburst.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.series-sunburst.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
[series-sunburst.data](option.series-sunburst.md#data) 的数据格式是树状的，例如：
...
点击此节点可跳转的超链接。须 [series-sunburst.nodeClick](option.series-treemap.md#nodeClick) 值为 `'link'` 时才生效。

参见 [series-sunburst.data.target](option.series-sunburst.md#data.target)。
...
意义同 HTML `<a>` 标签中的 `target`，参见 [series-sunburst.data.link](option.series-sunburst.md#data.link)。可选值为：`'blank'` 或 `'self'`。
...
**优先级：[series.data.label](option.series-sunburst.md#data.label) > [series.levels.label](option.series-sunburst.md#levels.label) > [series.label](option.series-sunburst.md#label)。**
...
同 [label.align](../option.md#sunburst.label.align)
...
可以在 [series.itemStyle](option.series-sunburst.md#itemStyle) 定义所有扇形块的样式，也可以在 [series.levels.itemStyle](option.series-sunburst.md#levels.itemStyle) 定义每一层扇形块的样式，还可以在 [series.data.itemStyle](option.series-sunburst.md#data.itemStyle) 定义每个扇形块单独的样式，这三者的优先级从低到高。也就是说，如果定义了 [series.data.itemStyle](option.series-sunburst.md#data.itemStyle)，将会覆盖 [series.itemStyle](option.series-sunburst.md#itemStyle) 和 [series.levels.itemStyle](option.series-sunburst.md#levels.itemStyle)。

**优先级：[series.data.itemStyle](option.series-sunburst.md#data.itemStyle) > [series.levels.itemStyle](option.series-sunburst.md#levels.itemStyle) > [series.itemStyle](option.series-sunburst.md#itemStyle)。**
...
用于指定旭日图扇形区块的内外圆角半径，支持设置固定数值或者相对于扇形区块的半径的百分比值。
...
图形的颜色。 默认从全局调色盘 [option.color](../option.md#color) 获取颜色。从 v5.2.0 开始支持在 `emphasis` 状态中支持设置为 `'inherit'` 取消高亮颜色。
...
子节点，递归定义，格式同 [series-sunburst.data](option.series-sunburst.md#data)。
...
*   `'link'`：如果节点数据中有 [link](option.series-sunburst.md#data.link) 点击节点后会进行超链接跳转。

## sort
- **Type**: `string|Function`
- **Default**: `'desc'`

扇形块根据数据 [`value`](option.series-sunburst.md#data.value) 的排序方式，如果未指定 `value`，则其值为子元素 `value` 之和。默认值 `'desc'` 表示降序排序；还可以设置为 `'asc'` 表示升序排序；`null` 表示不排序，使用原始数据的顺序；或者用回调函数进行排列：
...
淡出状态配置。开启 [emphasis.focus](option.series-sunburst.md#emphasis.focus) 后有效。
...
选中状态配置。开启 [selectedMode](option.series-sunburst.md#selectedMode) 后有效。
