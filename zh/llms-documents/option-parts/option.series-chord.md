...
和弦图的中心（圆心）坐标，数组的第一项是横坐标，第二项是纵坐标。
...
指定另一个坐标系组件，本 `series-chord` 布局在那个坐标系中。
...
*   `'calendar'`
    ...
    布局在一个 [日历坐标系](option.calendar.md) 中。当一个 ECharts 实例中存在多个日历坐标系时，须通过 [calendarIndex](option.series-chord.md#calendarIndex) 或 [calendarId](option.series-chord.md#calendarId) 指定所使用的日历坐标系。
    

*   `'matrix'`
    
    布局在一个 [矩阵坐标系](option.matrix.md)中。当一个 ECharts 实例中存在多个矩阵坐标系时，须通过 [matrixIndex](option.series-chord.md#matrixIndex) 或 [matrixId](option.series-chord.md#matrixId) 指定所使用的矩阵坐标系。
...
也参见 [series-chord.coordinateSystemUsage](option.series-chord.md#coordinateSystemUsage)。
...
如何在指定的 [坐标系](option.series-chord.md#coordinateSystem) 上布局本 `series-chord`。
...
*   `'box'`：**（不适用于 [series-chord](option.series-chord.md)）**
    
    此系列或组件作为一个整体，在指定的坐标系中进行布局——即根据坐标系计算整体的包围盒或基础锚点。
    
    *   例如，[grid 组件](option.grid.md) 可以布局在 [matrix 坐标系](option.matrix.md) 或 [calendar 坐标系](option.calendar.md) 中，这时其布局矩形是由 [series-chord.coords](option.series-chord.md#coords) 在坐标系中计算出来的。参见示例：[矩阵中的微型折线图](https://echarts.apache.org/examples/zh/editor.html?c=matrix-sparkline&edit=1&reset=1)。
...
另参考：[series-chord.coordinateSystem](option.series-chord.md#coordinateSystem)。
...
当 [coordinateSystemUsage](option.series-chord.md#coordinateSystemUsage) 为 `'box'` 时, `coord` 被输入给坐标系，计算得到布局位置（布局盒或者中心点）。
...
> 注：当 [coordinateSystemUsage](option.series-chord.md#coordinateSystemUsage) 为 `'data'` 时，输入给坐标系的是 `series.data[i]` 而非此 `coord`。
...
用于指定和弦图扇形区块的内外圆角半径，支持设置固定数值或者相对于扇形区块的半径的百分比值。
