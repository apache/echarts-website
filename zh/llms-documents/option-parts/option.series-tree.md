...
**注意：节点默认使用 `'emptyCircle'` 这个空心标记。在这种默认标记下，节点中心的填充色用于表示当前节点是否还有已折叠的子树，而节点外圈颜色由 [itemStyle.color](option.series-tree.md#itemStyle.color) 控制。如果配置自定义图片作为节点标记，ECharts 无法再通过这个中心填充色来切换折叠/展开状态，目前也不支持自动为折叠和展开状态分别指定两张不同图片。所以，如果希望明确显示节点的两种状态，建议使用 `'emptyCircle'` 这类内置标记类型。**
...
*   [itemStyle.color](option.series-tree.md#itemStyle.color) 控制的是节点外圈颜色。
*   节点中心的填充色用于表示当前节点是否还有已折叠的子树。
*   [itemStyle.borderColor](option.series-tree.md#itemStyle.borderColor) 和 [itemStyle.borderWidth](option.series-tree.md#itemStyle.borderWidth) 不会像实心标记那样直接控制外圈边框。

如果希望 [itemStyle.color](option.series-tree.md#itemStyle.color) 表现为常规填充色，并使用 [itemStyle.borderColor](option.series-tree.md#itemStyle.borderColor) / [itemStyle.borderWidth](option.series-tree.md#itemStyle.borderWidth) 来控制边框样式，可以将 [symbol](option.series-tree.md#symbol) 设置为 `'circle'` 这类实心标记。
