...
图表的分组，用于[联动](api.echarts.md#connect)
...
设置图表实例的配置项以及数据，万能接口，所有参数和数据的修改都可以通过 `setOption` 完成，ECharts 会合并新的参数和数据，然后刷新图表。如果开启[动画](../option.md#option.animation)的话，ECharts 找到两组数据之间的差异然后通过合适的动画去表现数据的变化。
...
*   `theme` 为 `string` 时，表示的是使用在图表实例初始化前调用 [echarts.registerTheme](api.echarts.md#registerTheme) 注册主题时使用的 `themeName`；当其为 `Object` 时，表示一个匿名的主题对象，并且直接使用该主题。
...
触发图表行为，例如图例开关`legendToggleSelect`, 数据区域缩放`dataZoom`，显示提示框`showTip`等等，更多见 [action](api.action.md) 和 [events](api.events.md) 的文档。
...
ECharts 中的事件有两种，一种是鼠标事件，在鼠标点击某个图形上会触发，还有一种是 调用 [dispatchAction](api.echartsInstance.md#dispatchAction) 后触发的事件。每个 action 都会有对应的事件，具体见 [action](api.action.md) 和 [events](api.events.md) 的文档。

如果事件是外部 [dispatchAction](api.echartsInstance.md#dispatchAction) 后触发，并且 action 中有 batch 属性触发批量的行为，则相应的响应事件参数里也会把属性都放在 batch 属性中。
...
转换坐标系或数轴或系列（即，[cartesian2d (grid)](../option-parts/option.grid.md), [only xAxis](../option-parts/option.xAxis.md), [only yAxis](../option-parts/option.yAxis.md), [polar](../option-parts/option.polar.md), [geo](../option-parts/option.geo.md), [series.map](../option-parts/option.series-map.md), [singleAxis](../option-parts/option.singleAxis.md), [calender](../option-parts/option.calendar.md), [matrix](../option-parts/option.matrix.md)）上的坐标到画布上的像素值。

输入的 `coord` 以及返回值的格式由各个坐标系或坐标轴或系列定义：

*   [cartesian2d（grid）](../option-parts/option.grid.md)：
    
    输入的 `coord` 是一个两项数组，其中 `value[0]` 和 `value[1]` 分别对应 [xAxis](../option-parts/option.xAxis.md) 和 [yAxis](../option-parts/option.yAxis.md)。数据类型根据 [axis.type](../option.md#~xAxis.type) 的不同而不同：
    
    *   如果 [axis.type](../option.md#~xAxis.type) 为 `'value'`、`'log'`，则输入的 `coord` 应为 `[number, number]`。
    *   如果 [axis.type](../option.md#~xAxis.type) 为 `'category'`，则输入的 `coord` 可以是 `[(number | string), (number | string)]`，其中 `string` 表示 `series.data` 中的原始字符串，`number` 表示由原始字符串转换而来的序号（为非负整数，从 `0` 开始递增）。
    *   如果 [axis.type](../option.md#~xAxis.type) 为 `'time'`，则输入的 `coord` 可以是 `[(number | string | Date), (number | string | Date)]`，其中 `number` 表示时间戳，`string | Date` 表示可被 [`echarts/src/util/number.ts` 中的 `parseDate` 方法](https://github.com/apache/echarts/blob/master/src/util/number.ts) 解析的任意时间格式。
...
*   [只转化 yAxis](../option-parts/option.xAxis.md) 或 [只转换 yAxis](../option-parts/option.yAxis.md)：
...
*   [polar](../option-parts/option.polar.md)：
    
    可类比于 `cartesian2d（grid）`。但仅支持通过 `polarIndex` / `polarId` / `polarName` 查询，不支持通过 `angleAxis` 和 `radiusAxis` 查询。
    
*   [geo](../option-parts/option.geo.md)：
    
    输入的 `coord` 可以是 `[number, number]`，当 [map](../option-parts/option.geo.md#map) 为 `GeoJSON` 时表示 `[longitude, latitude]`，当 [map](../option-parts/option.geo.md#map) 为 `SVG` 时表示 `[x_on_SVG, y_on_SVG]`。
    
    它也可以是一个 `string`，表示 `GeoJSON` 中的 `name`（即 `features[i].properties.name`，详见 [registerMap](api.echarts.md#registerMap)）或 `SVG` 中的名称（即参见 [SVG Base Map](https://echarts.apache.org/handbook/zh/how-to/component-types/geo/svg-base-map) 中的 “Named Element”），并返回该区域中心的像素点。
    ...
    在地理坐标系（[geo](../option-parts/option.geo.md)）上，把某个点的经纬度坐标转换成为像素坐标：
...
*   [series.map](../option-parts/option.series-map.md)：
...
*   [singleAxis](../option-parts/option.singleAxis.md)：
...
*   [calendar](../option-parts/option.calendar.md)：
...
*   In [matrix](../option-parts/option.matrix.md)：
    ...
    输入的 `coord` 用于定位一个矩形，然后返回该矩形的中心点。更多详情参见 [matrix.body.data.coord](../option-parts/option.matrix.md#body.data.coord)。例如：
...
*   [series.graph](../option-parts/option.series-graph.md)：
...
将坐标系上的 `coord`（即，[calendar](../option-parts/option.calendar.md)、[matrix](../option-parts/option.matrix.md)）转换为布局信息。
...
转换像素值到逻辑坐标系上的点。是 [convertToPixel](api.echartsInstance.md#convertToPixel) 的逆运算。

具体实例可参考 [convertToPixel](api.echartsInstance.md#convertToPixel)。
...
目前支持在这些坐标系和系列上进行判断：[grid](../option-parts/option.grid.md), [polar](../option-parts/option.polar.md), [geo](../option-parts/option.geo.md), [matrix](../option-parts/option.matrix.md), [series-map](../option-parts/option.series-map.md), [series-graph](../option-parts/option.series-graph.md), [series-pie](../option-parts/option.series-pie.md)。
...
显示加载动画效果。可以在加载数据前手动调用该接口显示加载动画，在数据加载完成后调用 [hideLoading](api.echartsInstance.md#hideLoading) 隐藏加载动画。
...
*   现在不支持 [系列（series）](../option.md#series) 使用 [dataset](../option-parts/option.dataset.md) 同时使用 `appendData`，只支持系列使用自己的 [series.data](../option.md#series.data) 时使用 `appendData`。
*   目前并非所有的图表都支持分片加载时的增量渲染。目前支持的图有：ECharts 基础版本的 [散点图（scatter）](../option-parts/option.series-scatter.md) 和 [线图（lines）](../option-gl.md#series-linesGL)。ECharts GL 的 [散点图（scatterGL）](../option-gl-parts/option-gl.series-scatterGL.md)、[线图（linesGL）](../option-gl-parts/option-gl.series-lines3D.md) 和 [可视化建筑群（polygons3D）](../option-gl-parts/option-gl.series-polygons3D.md)。
